...
OUT_DIR="${HOTSPOT_RIFT_OUT_DIR:-/private/tmp/rift-hotspot-region-smoke}"
...
"${JAVAC_BIN}" \
  ...
  -d "${OUT_DIR}/classes" \
  "${SRC_DIR}/RiftHotSpotRegionSmoke.java"

"${JAVA_BIN}" \
  --add-exports java.base/jdk.internal.rift=ALL-UNNAMED \
  -cp "${OUT_DIR}/classes" \
  RiftHotSpotRegionSmoke disabled
