...
OUT_DIR="${HOTSPOT_RIFT_OUT_DIR:-/private/tmp/rift-hotspot-c1-region-smoke}"
...
"${JAVAC_BIN}" \
  ...
  -d "${OUT_DIR}/classes" \
  "${SRC_DIR}/RiftHotSpotC1RegionAllocationSmoke.java"
...
"${JAVA_BIN}" \
  ...
  -XX:CompileCommand=compileonly,RiftHotSpotC1RegionAllocationSmoke.allocateOne \
  --add-exports java.base/jdk.internal.rift=ALL-UNNAMED \
  -cp "${OUT_DIR}/classes" \
  RiftHotSpotC1RegionAllocationSmoke
