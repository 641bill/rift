...
same-shape/L1 integration follow-up at child `bcbfe80f5`. It records which higher-level operators may
...
| Append/pop top-k | Yak topword, LogHub top templates, Common Crawl top domains | `RegionPriorityQueue` validates checked owner-token top-k safety. LogHub top-template retained controls passed strongly: generated 1M checked scoped retained is `290.610 ms` versus retained heap `424.443 ms`, and real HDFS preloaded 1M checked scoped retained is `81.174 ms` versus retained heap `116.138 ms`. The reusable `EpochTopKByKey` API also passes the same gate. After the increment hot-path follow-up, generated 1M checked scoped top-k is `274.914 ms` versus retained heap `397.788 ms`, and real HDFS preloaded checked scoped top-k is `82.170 ms` versus retained heap `111.704 ms`. The L1 final-clean real HDFS x20 row confirms the direction: reusable checked top-k scoped `4.88 s` and `28 MB` RSS versus retained heap `5.52 s` and `205 MB` RSS. Yak topword now adds a second natural top-k workload. In L2, `checked-epoch-topk-scoped` is `249.311 ms` versus natural heap `313.724 ms` and same-shape heap top-k retained `297.740 ms`; in L1, it is `4.94 s` versus natural heap `6.25 s` and same-shape heap top-k retained `5.72 s`, with region-like RSS. | The reusable API now has small measured overhead versus the benchmark-local count-array path: generated same-run overhead is about `5.7%` (`274.914 ms` vs `260.130 ms`), real HDFS L2 overhead is about `7.0%` (`82.170 ms` vs `76.818 ms`), and real HDFS L1 overhead is about `1.7%` (`4.88 s` vs `4.80 s`). Yak topword also shows a topology caveat: the older checked close-traversal epoch row is still faster than reusable top-k in both L2 (`234.031 ms` vs `249.311 ms`) and L1 (`4.61 s` vs `4.94 s`) for that local workload. | Passed reusable retained top-k API gate with materially reduced overhead, L1 real-input confirmation, and a second generated-methodology top-k confirmation. | Keep `heap-retained-drop-anchor` or same-shape heap top-k controls in every top-k row. The next allowed action is application integration only for natural top-k workloads, plus profiling if a future row exposes API overhead or a topology loss. |
...
| Hash/group aggregate | Dataflow AGGREGATE, LogHub sessions, Common Crawl domains, NEXMark Q5 | Direct epoch and retained-epoch rows already cover summary-on-append aggregate shapes. Generic `StreamWindowFold` now narrowly passes its focused 1M gate after redundant open-check removal (`97.321 ms` checked versus `99.302 ms` heap), with lower RSS and zero checked GC. The first application-style gate also passes: NEXMark q5 `rift-checked-fold-api` is `419.855 ms` versus heap `470.288 ms`, with matching checksum/output, zero checked GC, and RSS `219.5 MB` versus heap `289.9 MB`. | Same-shape heap for any new hash-table-based API; retained controls if records survive to close; Beam-default or real-input reruns before broader claims. | Fold is now a generated-methodology application candidate for Q5 and a focused operator win; still keep direct epoch or page-token for simpler additive summaries. | Rerun Q5 with Beam defaults or move to the next fold-shaped application only if the semantics require generic fold state. |
