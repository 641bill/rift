...
commit `9abac4833`. Child `3598efe29` adds L1 final-clean support for the
same top-k row and records a real HDFS representative result. It records which
higher-level operators may be optimized or integrated into application
benchmarks under the new comparison-class rules.
...
| Append/pop top-k | Yak topword, LogHub top templates, Common Crawl top domains | `RegionPriorityQueue` validates checked owner-token top-k safety. LogHub top-template retained controls passed strongly: generated 1M checked scoped retained is `290.610 ms` versus retained heap `424.443 ms`, and real HDFS preloaded 1M checked scoped retained is `81.174 ms` versus retained heap `116.138 ms`. The reusable `EpochTopKByKey` API also passes the same gate: generated 1M checked scoped top-k is `341.905 ms` versus retained heap `463.578 ms`; real HDFS preloaded checked scoped top-k is `95.267 ms` versus retained heap `123.024 ms`. The L1 final-clean real HDFS x20 row confirms the direction: reusable checked top-k scoped `5.05 s` and `28 MB` RSS versus retained heap `5.46 s` and `205 MB` RSS. | The reusable API has measurable abstraction overhead versus the benchmark-local count-array path (`341.905 ms` vs `300.984 ms` generated; L1 real HDFS `5.05 s` vs benchmark-local checked `4.84 s`). It still needs profiling/inlining before being used as a final lower-bound operator. | Passed reusable retained top-k API gate with overhead caveat and L1 real-input confirmation. | Profile or inline the `EpochTopKByKey` update/getter path before integrating into new applications; keep `heap-retained-drop-anchor` controls in every top-k row. |
