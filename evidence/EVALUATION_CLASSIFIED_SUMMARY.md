...
| StreamFlex design throughput | generated methodology / object-retained stream design | framework API win / L1 final-clean | `gc-heap` and `heap-same-shape` both `1.52 s`, RSS `12.4 MB` | `checked-epoch-scoped` `1.27 s`, RSS `7.9 MB`; `checked-epoch-stream` `1.33 s`, RSS `8.0 MB` | `checked-epoch-scoped` is `16.4%` faster than heap | L2: heap median GC `100.234 ms`; checked scoped median GC `0 ms` | about `-36%` RSS | Rift-native StreamFlex stable/transient/capsule throughput evidence; not exact StreamFlex/Ovm artifact reproduction. |
