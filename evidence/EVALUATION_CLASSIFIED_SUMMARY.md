...
versus rooted scoped `0.21 s` and heap `0.51 s`.
...
| SPECjbb2005 workload port, 4 warehouses | clean-room transaction workload port | quick all-optimizations L1 gate | heap `0.51 s`, RSS `7.9 MB` | checked epoch stream `0.18 s`, RSS `6.0 MB`; checked epoch scoped `0.19 s`; rooted scoped `0.21 s` | checked stream `64.7%` faster than heap and `14.3%` faster than rooted scoped | L2 interpretation remains the 4w scale row above: heap GC `7.389 ms`, checked rows `0 ms` | about `-25%` vs heap | Quick L1 check that current optimized defaults preserve the transaction/epoch win; not official SPECjbb2005 and not a replacement for the 8w representative row. |
