...
throughput/RSS/GC/fixed-memory row over public Wikimedia data.
...
| Wikimedia enwiki clickstream retained session 1M | real-streaming-input Wikimedia clickstream gzip stream | retained session / fixed-memory evidence | L1 heap `6.50 s`, RSS `783.7 MB`; L2 heap `2038.262 ms`, GC `150.157 ms`, max GC `178.978 ms`; heap caps fail at `512M`, `256M`, `128M`, and `64M` | L1 checked Rift `5.81 s`, RSS `137.9 MB`; L2 checked Rift `1952.438 ms`, GC `9.237 ms`, region op `1.871 ms`; checked rows pass `128M` and `64M` caps | L1 checked Rift `10.6%` faster; L2 checked Rift `4.2%` faster | L2 median `-140.920 ms`; heap GC is about `7.4%` of L2 | L1 about `-82%` | Named real compressed-stream retained clickstream workload over public Wikimedia data. Use as throughput/RSS/GC/fixed-memory evidence; not an official Wikimedia benchmark artifact. |
