...
| Theodolite power q2 streaming-file | real-streaming-input UCI household power-meter trace | best checked topology / L1 final-clean plus L2 standard stats | L1 heap `3.87 s`, RSS `75.3 MB`; L2 heap `1007.030 ms`, GC `19.932 ms` | L1 `checked-epoch-scoped` `3.77 s`, RSS `24.9 MB`; L2 checked scoped `967.144 ms`, GC `0 ms` | L1 `2.6%` faster; L2 `4.0%` faster | L2 median `-19.932 ms` | L1 about `-67%` | Real-streaming-input Theodolite row after replacing `String.split` with a byte-field cursor. Checked scoped epoch is a modest throughput/RSS/fixed-memory win over heap. The older parser-heavy streaming row is superseded because it inflated GC (`143.088 ms` heap GC) with parser allocation. Older preloaded/full-local row remains a control (`5.45 s` checked vs `5.46 s` heap). |
