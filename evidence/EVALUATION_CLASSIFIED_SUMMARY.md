...
| Wikimedia enwiki clickstream retained session 1M/5M/10M | real-streaming-input Wikimedia clickstream gzip stream | retained session / fixed-memory evidence | 1M L1 heap `6.50 s`, RSS `783.7 MB`; L2 heap `2038.262 ms`, GC `150.157 ms`. 5M L1 heap `28.77 s`, RSS `1.54 GB`; L2 heap `9459.416 ms`, GC `334.176 ms`, max GC `1066.487 ms`; 5M heap caps fail at `1G`, `768M`, and `512M`. 10M x3 L1 heap `62.52 s`, RSS `1.80 GB`; L2 heap `20103.863 ms`, GC `851.023 ms`, max GC `885.434 ms`; heap caps fail at `1G`, `768M`, and `512M` | 1M L1 checked Rift `5.81 s`, RSS `137.9 MB`; L2 checked Rift `1952.438 ms`, GC `9.237 ms`, region op `1.871 ms`. 5M L1 checked Rift `27.86 s`, RSS `138.4 MB`; L2 checked Rift `9486.647 ms`, GC `46.492 ms`, region op `10.699 ms`. 10M x3 L1 checked Rift `59.37 s`, RSS `138.4 MB`; L2 checked Rift `19635.995 ms`, GC `96.069 ms`, region op `24.329 ms`; checked rows pass `64M` cap | 1M L1 checked Rift `10.6%` faster; 5M L1 `3.2%` faster; 10M x3 L1 `5.0%` faster; 5M L2 essentially tied; 10M L2 `2.3%` faster | 1M L2 median `-140.920 ms`; 5M L2 median `-287.684 ms`, max `-1019.496 ms`; 10M x3 median `-754.954 ms` | 1M L1 about `-82%`; 5M L1 about `-91%`; 10M x3 about `-92%` | Named real compressed-stream retained clickstream workload over public Wikimedia data. 1M is throughput/RSS/GC/fixed-memory evidence; 5M strengthens RSS/fixed-memory and GC-tail evidence; 10M is report-grade real-streaming retained clickstream scale-up evidence. Not an official Wikimedia benchmark artifact. |
