...
The 10M one-run feasibility probe is stronger again on material GC pressure:
...
while checked rows pass under a `64M` GC heap cap. Treat this as provisional
feasibility evidence until a 3-run median rerun.
...
| Wikimedia enwiki clickstream retained session 1M/5M/10M | real-streaming-input Wikimedia clickstream gzip stream | retained session / fixed-memory evidence | 1M L1 heap `6.50 s`, RSS `783.7 MB`; L2 heap `2038.262 ms`, GC `150.157 ms`. 5M L1 heap `28.77 s`, RSS `1.54 GB`; L2 heap `9459.416 ms`, GC `334.176 ms`, max GC `1066.487 ms`; 5M heap caps fail at `1G`, `768M`, and `512M`. 10M one-run heap `24.23 s`, RSS `1.16 GB`; L2 heap `22877.820 ms`, GC `4058.998 ms`; heap caps fail at `1G`, `768M`, and `512M` | 1M L1 checked Rift `5.81 s`, RSS `137.9 MB`; L2 checked Rift `1952.438 ms`, GC `9.237 ms`, region op `1.871 ms`. 5M L1 checked Rift `27.86 s`, RSS `138.4 MB`; L2 checked Rift `9486.647 ms`, GC `46.492 ms`, region op `10.699 ms`. 10M one-run checked Rift `19.96 s`, RSS `136.1 MB`; L2 checked Rift `19716.840 ms`, GC `99.708 ms`, region op `31.049 ms`; checked rows pass `64M` cap | 1M L1 checked Rift `10.6%` faster; 5M L1 `3.2%` faster; 10M one-run `17.6%` faster; 5M L2 essentially tied; 10M L2 `13.8%` faster | 1M L2 median `-140.920 ms`; 5M L2 median `-287.684 ms`, max `-1019.496 ms`; 10M one-run median `-3959.290 ms` | 1M L1 about `-82%`; 5M L1 about `-91%`; 10M one-run about `-88%` | Named real compressed-stream retained clickstream workload over public Wikimedia data. 1M is throughput/RSS/GC/fixed-memory evidence; 5M strengthens RSS/fixed-memory and GC-tail evidence; 10M is promising material-GC feasibility evidence pending 3-run median rerun. Not an official Wikimedia benchmark artifact. |
