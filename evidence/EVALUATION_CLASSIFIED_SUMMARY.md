...
| Theodolite power q2 | real UCI household power-meter trace | ceiling/control / L1 final-clean plus L2 standard stats | L1 heap `5.46 s`, RSS `307 MB`; L2 heap `287.296 ms`, GC `20.354 ms` | L1 `checked-epoch-scoped` `5.45 s`, RSS `304 MB`; L2 checked scoped `288.495 ms`, GC `0 ms` | L1 effectively tied/slightly faster; L2 tied/slightly slower | L2 median `-20.354 ms` | L1 about `-1%` | Real-input Theodolite-style control row. Checked scoped epoch removes visible heap GC and ties elapsed/RSS, but parser/file CPU dominates and it is not a GC-heavy flagship. |
