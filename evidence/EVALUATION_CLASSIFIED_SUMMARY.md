...
The same retained-session harness now has a named Wikimedia clickstream
workload. It derives session keys from source article, target article, and
link kind, and uses click count as the value. At 1M streamed rows, L1 checked
...
`150.157 ms`. Heap fails even at a `512M` cap, while checked rows complete
under `128M` and `64M`. This is now a real-streaming retained clickstream
throughput/RSS/GC/fixed-memory row over public Wikimedia data. The 5M
scale-up keeps checked Rift L1 slightly faster (`27.86 s` versus heap
`28.77 s`) and cuts RSS from `1.54 GB` to `138 MB`. Its L2 loop is essentially
tied (`9486.647 ms` versus heap `9459.416 ms`), but heap max GC grows to
`1066.487 ms` and heap fails at `1G`, `768M`, and `512M`, while checked rows
complete under a `64M` GC heap cap. Treat the 5M row primarily as scale-up
RSS/fixed-memory and GC-tail evidence.
...
and region op `24.329 ms`, versus heap `20103.863 ms`, GC `851.023 ms`.
Heap fails at `1G`, `768M`, and `512M`, while checked rows pass under a
`64M` GC heap cap.
The full local compressed file feasibility pass processes all `35862259`
rows in one run. Checked Rift is L1 `69.87 s`, RSS `136 MB`, versus heap
`74.23 s`, RSS `2.30 GB`; L2 checked Rift is `71149.579 ms`, GC
`318.165 ms`, and region op `79.786 ms`, versus heap `72750.014 ms`, GC
`7122.811 ms`. Treat this as provisional full-input feasibility until the
full-file row is repeated; the 10M x3 row remains the report-grade median.
...
| Wikimedia enwiki clickstream retained session 1M/5M/10M/full | real-streaming-input Wikimedia clickstream gzip stream | retained session / fixed-memory evidence | 1M L1 heap `6.50 s`, RSS `783.7 MB`; L2 heap `2038.262 ms`, GC `150.157 ms`. 5M L1 heap `28.77 s`, RSS `1.54 GB`; L2 heap `9459.416 ms`, GC `334.176 ms`, max GC `1066.487 ms`; 5M heap caps fail at `1G`, `768M`, and `512M`. 10M x3 L1 heap `62.52 s`, RSS `1.80 GB`; L2 heap `20103.863 ms`, GC `851.023 ms`, max GC `885.434 ms`; heap caps fail at `1G`, `768M`, and `512M`. Full one-run heap `74.23 s`, RSS `2.30 GB`; L2 heap `72750.014 ms`, GC `7122.811 ms` | 1M L1 checked Rift `5.81 s`, RSS `137.9 MB`; L2 checked Rift `1952.438 ms`, GC `9.237 ms`, region op `1.871 ms`. 5M L1 checked Rift `27.86 s`, RSS `138.4 MB`; L2 checked Rift `9486.647 ms`, GC `46.492 ms`, region op `10.699 ms`. 10M x3 L1 checked Rift `59.37 s`, RSS `138.4 MB`; L2 checked Rift `19635.995 ms`, GC `96.069 ms`, region op `24.329 ms`; checked rows pass `64M` cap. Full one-run checked Rift `69.87 s`, RSS `136 MB`; L2 `71149.579 ms`, GC `318.165 ms`, region op `79.786 ms` | 1M L1 checked Rift `10.6%` faster; 5M L1 `3.2%` faster; 10M x3 L1 `5.0%` faster; full one-run L1 `5.9%` faster; 5M L2 essentially tied; 10M L2 `2.3%` faster; full one-run L2 `2.2%` faster | 1M L2 median `-140.920 ms`; 5M L2 median `-287.684 ms`, max `-1019.496 ms`; 10M x3 median `-754.954 ms`; full one-run median `-6804.646 ms` | 1M L1 about `-82%`; 5M L1 about `-91%`; 10M x3 about `-92%`; full one-run about `-94%` | Named real compressed-stream retained clickstream workload over public Wikimedia data. 1M is throughput/RSS/GC/fixed-memory evidence; 5M strengthens RSS/fixed-memory and GC-tail evidence; 10M is report-grade real-streaming retained clickstream scale-up evidence. Full-file one-run over all `35862259` rows confirms feasibility but is provisional until repeated. Not an official Wikimedia benchmark artifact. |
...
- **Real-input claim:** Yak LiveJournal is the strongest real-input epoch row,
  ...
  removing timed heap GC. Wikimedia enwiki clickstream now adds a named
  compressed retained-session row with checked-Rift throughput/RSS/GC wins and
  heap-cap failure even at `512M`. Yak generated topword provides a second generated
