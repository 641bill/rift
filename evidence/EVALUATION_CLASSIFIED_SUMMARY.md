...
| LogHub HDFS q2, Spark/Windows q3, DSPBench Log q2, GH Archive q1/q2, Theodolite power q2 | complete where used; Spark q3 L1 elapsed not promoted because `/usr/bin/time` real field was anomalous | complete | real-input and real-streaming-input page/window/epoch modest/RSS/control rows | keep as real-input modest/control evidence; Theodolite streaming-file q2 is now a stronger RSS/fixed-memory row than the older preloaded control |
...
| StreamFlex design throughput | generated methodology / object-retained stream design | framework API win / L1 final-clean | 20M x3 L1 `gc-heap` `31.26 s`, RSS `12.4 MB` | `checked-epoch-scoped` `24.39 s`, RSS `12.6 MB`; `checked-epoch-stream` `22.81 s`, RSS `12.6 MB` | checked stream `27.0%` faster than heap; checked scoped `22.0%` faster | L2 1M: heap median GC `100.234 ms`; checked scoped median GC `0 ms` | near tie at 20M L1; older 1M L1 shows lower checked RSS | Rift-native StreamFlex stable/transient/capsule throughput evidence; not exact StreamFlex/Ovm artifact reproduction. |
...
| Common Crawl WET-shaped q1 | generated stream stressor | best checked topology / L1 final-clean | 1M x3 L1 heap `17.49 s`, RSS `409 MB`; L2 heap GC `1741.640 ms` from prior stats row | checked Rift page-token `10.88 s`, RSS `63 MB`; checked SafeZone-backed page-token `13.29 s`, RSS `63 MB` | checked Rift page-token `37.8%` faster than heap | L2 prior rows show material heap GC removal | L1 about `-85%` | Strong generated stream-object pressure win; not real-input proof. |
| Common Crawl WET-shaped q2 | generated stream/window stressor | best checked topology / L1 final-clean | 1M x3 L1 heap `16.73 s`, RSS `409 MB`; L2 heap GC `1565.074 ms` from prior stats row | checked Rift page-token `11.49 s`, RSS `63 MB`; checked SafeZone-backed page-token `14.84 s`, RSS `63 MB` | checked Rift page-token `31.3%` faster than heap | L2 prior rows show material heap GC removal | L1 about `-85%` | Strong generated window/object pressure win; not real-input proof. |
...
| Theodolite power q2 streaming-file | real-streaming-input UCI household power-meter trace | best checked topology / L1 final-clean plus L2 standard stats | L1 heap `10.92 s`, RSS `147 MB`; L2 heap `2479.703 ms`, GC `143.088 ms` | L1 `checked-epoch-scoped` `10.07 s`, RSS `29.6 MB`; L2 checked scoped `2436.713 ms`, GC `54.154 ms` | L1 `7.8%` faster; L2 `1.7%` faster | L2 median `-88.934 ms` | L1 about `-80%` | Real-streaming-input Theodolite row. Checked scoped epoch is a throughput/RSS/fixed-memory win over heap, but parser/string allocation still creates heap GC in all modes, so this is not a huge-GC flagship. Older preloaded/full-local row remains a control (`5.45 s` checked vs `5.46 s` heap). |
