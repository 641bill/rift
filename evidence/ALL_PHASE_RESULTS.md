...
| Phase 6 | Built methodology harnesses for Broom/StreamFlex/Yak/Stancu comparison axes, then added NEXMark-lite, Common Crawl WET-shaped, and Wikimedia pageview/clickstream stream-processing probes. | These support the broader research story but are not exact reproductions of closed or unavailable artifacts. NEXMark-lite Q1/Q2 show modest 1M elapsed wins and lower GC, generic Q8 is a checked join-window win, the reusable Q8 `StreamJoinWindow` API is lower-GC/lower-RSS but slower than a fair specialized heap control, and Q5 remains checked-container/window-overhead evidence rather than a window-aggregate win. Common Crawl and generated Wikimedia detect heap allocation pressure but do not clear the case-study gate against improved SafeZone/heap, so the next ladder candidate is Linear Road-style latency/toll processing. |
...
- Next stream-benchmark search should move to Linear Road-style position
