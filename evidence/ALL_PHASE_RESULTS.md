...
- The positive compiler/runtime probes store ordinary Scala row objects
  ...
  read the top row, remove the keys in the close callback, and then close the
  bucket.
...
| `StreamWindowIndexedRank` stores child-region objects, ranks them, and removes keys before bucket close | passes compiler probe and native runtime smoke |
...
- This validates the next general stream-operator shape above
  ...
  unlinked before bucket close.
- It is not a speed win. Checked Rift is slower on elapsed time even though
  Rift runtime operation time is low and RSS is lower. Treat this as API/safety
