...
| Final component selection and default-run pruning | Started | Syntax-validated runner/script update plus dirty-worktree smoke validation; no headline medians | `docs/FINAL_COMPONENT_SELECTION.md` now defines public candidates, internal controls, and gated/rejected operators. The parent performance runner plus the main sandbox matrix scripts now exclude current SafeZone and rootless/unsafe controls by default. Set `RIFT_EVAL_INCLUDE_CONTROLS=1`, `RIFT_BENCH_INCLUDE_CONTROLS=1`, or explicit `*_MODES` variables to reproduce lower-bound/control rows. Run `2026-05-06-final-selection-smoke` completed `preflight core prior checked streams reml` at smoke scale with `include_controls=0`; treat it as runner/default validation only because repos were dirty. |
