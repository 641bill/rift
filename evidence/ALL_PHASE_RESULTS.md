...
not a Rift-specific explanation for the checked loss.
...
| rift-checked | 118.726 | 0.000 | 0.175 | 1000004 | 40402944 | lower GC/RSS, failed speed gate |
...
- The focused gate failed because checked elapsed time is slower than heap at
  1M (`118.726 ms` versus `103.244 ms`).
- The memory result is still useful: checked removes measured GC, keeps Rift
  op time below `1 ms`, and cuts RSS by about `34.6 MB`.
- Per the gate, Common Crawl WET and NEXMark Q5 fold-backed integration should
  not start until fold table/API overhead is profiled and reduced.
...
- `NEXMARK_REGION_MATRIX.md` is now the first broader non-DEBS ladder for that
  ...
  `291.832 ms` versus heap `322.210 ms`. Q0 passthrough is a near tie, and Q5
...
- The Q5 diagnostic follow-up confirms identical window operation counts and
  ...
  per-entry/window-container overhead work rather than more broad benchmark
  expansion.
