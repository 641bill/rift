...
| SafeZone cost-decomposition scaffold | `evidence/SAFEZONE_COST_MATRIX.md` |
...
| SafeZone-family cost decomposition | Scaffold implemented; headline rows pending | Smoke validated only | `sandbox/run_safezone_cost_matrix.sh` records SafeZone pool trace counters (`SAFEZONE_TRACE=1`) alongside benchmark medians/RSS for root-mode and page-size configurations. Use this before optimizing SafeZone internals or adding `rift-checked-safezone-hp`. |
...
| Phase 6 | Built methodology harnesses for Broom/StreamFlex/Yak/Stancu comparison axes, then added NEXMark-lite, Beam-default NEXMark-profile rows, Common Crawl WET-shaped, Wikimedia pageview/clickstream, Linear Road-shaped, Yahoo-style ad-stream, and RIoTBench-style IoT stream-processing probes with generated and real/preloaded inputs where available. Clean headline stream sweeps now record 1M generated/profile rows for all current stream matrices, including an UnsafeZone-HP follow-up, and UnsafeZone-HP sweeps add a SafeZone-derived no-root substrate control across core/prior and bounded DEBS. The Common Crawl WET-shaped detector now has q2 domain-window and q3 parser-scratch probes pending headline measurement. | These support the broader research story but are not exact reproductions of closed or unavailable artifacts. In the clean stream sweep, NEXMark Beam-default Q3 is the best checked row (`292.371 ms` checked vs `316.626 ms` heap and `297.962 ms` improved SafeZone in the UnsafeZone follow-up), Q8 is a checked row below the case-study gate (`450.904 ms` vs heap `467.213 ms` and improved SafeZone `462.599 ms`), and Q11 is heap-fastest. Generated Common Crawl WET-shaped tokenization is the clearest GC-heavy detector (`1538.107 ms` heap GC at 1M generated pages), but UnsafeZone-HP and improved SafeZone beat trusted Rift on elapsed time. Yahoo, RIoTBench, Wikimedia, and Linear Road rows are mostly near-ties or heap/improved/Unsafe SafeZone wins. Across core/prior/stream/DEBS legs, SafeZone no-root internals usually beat or match current Rift HPZone, suggesting SafeZone allocator/pool mechanics may be the right substrate to learn from or build on. Cost decomposition is now the required next measurement before backend changes. |
...
| Common Crawl WET tokenization | Generated and real preloaded page/line/token streams with bucket-owned ordinary Scala records | Corrected generated 100k default tokenization stresses heap (`149.149 ms` GC, `408305664` RSS bytes), but improved SafeZone is fastest (`381.006 ms` vs Streaming `403.935 ms`). Real WET preloaded rows are heap-fastest with median-zero timed GC: 10k pages heap `12.079 ms` vs Streaming `15.651 ms`; 50k requested / 21425 actual pages heap `26.452 ms` vs Streaming `33.103 ms`. | Real preloaded WET does not clear the case-study gate; no checked mode yet. |
