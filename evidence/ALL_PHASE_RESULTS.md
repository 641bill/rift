...
| Cheap checked operator families | Started, first reusable APIs validated | Compile/tests plus focused 1M-shape 3-run and RSS rows | `PageTokenMapFilter`, `RegionList`, and `EpochBuffer` are now real reusable APIs. Focused rows are positive for SELECT, linked ListOfLists, and epoch append/drain: SELECT scoped page-token `18.214 ms` in the latest API rerun and `17.980 ms` / `30375936` RSS bytes in the direct-binary rerun vs heap `27.872-27.932 ms`; reusable `RegionList` ListOfLists builder `5927.385 ms` vs earlier heap around `14.8-15.4 s`; `EpochBuffer` 1M scoped checked `25.448 ms` and checked Rift `26.673 ms` vs heap epoch `27.164 ms` with `5.707 ms` GC. `EpochFold` is implemented and correct but failed its first speed gate at `91.938 ms`; the older `38.399 ms` aggregate row remains exact-array checked aggregate evidence, not true `EpochFold` evidence. Full comprehensive headline sweep remains pending. |
| StreamFlex checked EpochBuffer integration | Correct but negative | Compile, smoke, and 200k throughput rows | `StreamFlexRegionMatrix` now accepts `rift-checked-epoch-buffer` and `rift-checked-safezone-epoch-buffer`, and its default runner omits current SafeZone. Checksums match. At 200k throughput, heap is `42.504 ms` with `8.942 ms` GC, improved SafeZone `40.224 ms`, Rift Streaming `36.357 ms`, checked EpochBuffer `47.462 ms`, and scoped checked EpochBuffer `44.504 ms`. This shows `EpochBuffer` is too granular for multi-stage pipelines when stacked four times per batch; next operator should be `TransactionRegion`/multi-list epoch. |
