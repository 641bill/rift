...
| Runtime substrate control: UnsafeZone-HP | Headline core/prior medians started | Validated for covered local harnesses; unsafe by design | SafeZone no-root mode `SAFEZONE_ROOTS_MODE=3` with 32 KiB pages. Clean core/prior sweep shows it usually beats improved SafeZone slightly and beats current Rift HPZone on linked/prior-work rows, while Rift HPZone still wins flat ListOfLists. Stream and DEBS headline medians with this mode remain open. |
...
| Phase 6 | Built methodology harnesses for Broom/StreamFlex/Yak/Stancu comparison axes, then added NEXMark-lite, Beam-default NEXMark-profile rows, Common Crawl WET-shaped, Wikimedia pageview/clickstream, Linear Road-shaped, Yahoo-style ad-stream, and RIoTBench-style IoT stream-processing probes with generated and real/preloaded inputs where available. A clean headline stream sweep now records 1M generated/profile rows for all current stream matrices, and an UnsafeZone-HP core/prior sweep adds a SafeZone-derived no-root substrate control. | These support the broader research story but are not exact reproductions of closed or unavailable artifacts. In the clean stream sweep, NEXMark Beam-default Q3 is the best checked row (`295.166 ms` checked vs `315.715 ms` heap and `302.668 ms` improved SafeZone), Q8 is a checked near-tie with improved SafeZone, and Q11 is heap-fastest. Generated Common Crawl WET-shaped tokenization is the clearest GC-heavy detector (`1559.601 ms` heap GC at 1M generated pages), but improved SafeZone beats trusted Rift on elapsed time. Yahoo, RIoTBench, Wikimedia, and Linear Road rows are mostly near-ties or heap/improved-SafeZone wins. In the UnsafeZone-HP core/prior leg, SafeZone no-root internals beat improved SafeZone slightly on many rows and usually beat current Rift HPZone, suggesting SafeZone allocator/pool mechanics may be the right substrate to learn from or build on. |
...
- Improved SafeZone is a serious baseline and cannot be omitted. UnsafeZone-HP
  ...
  faster than the current Rift backend, but that mode is unsafe and
  benchmark-only.
