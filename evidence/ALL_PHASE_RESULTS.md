...
evidence, not the missing GC-heavy case. LogHub HDFS v1 is now added as a
larger real log input with `11175629` lines. Its strongest 1M row is
q2-window-counts: checked scoped page-token is `7871.856 ms` vs heap
`8227.369 ms`, removes heap median GC `92.659 ms`, and lowers RSS from
`408666112` to `394969088`. HDFS q2 is the strongest checked LogHub row so
far, but still modest because heap GC is about 1% of elapsed. RIoTBench/MHEALTH
is now wired as a provenance-clean real IoT input: the UCI MHEALTH dataset has
`1215745` rows, and 1M q1/q2 medians matched checksums but showed zero timed
heap GC. q1 is a near-tie with heap fastest (`117.977 ms`) and q2 gives a
small SafeZone win (`107.194 ms` vs heap `109.589 ms`). Treat it as a
ceiling/control row. A Yak real-input `graphreal` row is now implemented over
SNAP Twitter ego and LiveJournal edge lists. Twitter ego proved the path; the
stronger LiveJournal follow-up now includes checked topology rows. At 50M
replayed real edges, `gc-heap` is `1618.105 ms`, median timed GC is
`273.410 ms`, and RSS is `2761261056`. The low-RSS epoch rows are the best
safe/topology signal: `checked-epoch-scoped` is `1069.241 ms`,
`checked-epoch-stream` is `1113.261 ms`, `region-scoped-rooted` is
`1256.538 ms`, and `region-stream-rootless` is `1345.479 ms`, all with about
`1.53 GB` RSS and zero timed GC. Whole-run checked scoped is faster
(`1048.751 ms`) but reaches `2977742848` RSS because it does not reclaim until
the end. Page-token checked rows also beat heap but are slower for Yak
(`1403.878/1457.811 ms`). Reusable `EpochBuffer` is now wired into the same
path and beats heap in a 50M same-run comparison (`1343.071 ms` scoped versus
heap `1514.313 ms`), but trails the benchmark-local linked epoch path
(`1022.643 ms` scoped). `RiftRegion.epoch { ... }` now implements the direct
checked linked-epoch reusable API and passes smoke/tests; the 10M LiveJournal
API-backed rerun gives direct checked epoch `212.691/229.532 ms` scoped/stream
versus heap `317.779 ms` and `EpochBuffer` about `286 ms`; the apples-to-apples
50M API-backed rerun gives `checked-epoch-scoped` `1055.958 ms` and
`checked-epoch-stream` `1101.001 ms` versus heap `1604.811 ms` with
`288.801 ms` timed GC. The same reusable epoch topology now covers
`wordcount`, `graphstep`, `sort`, `topword`, and `graphchi`. The new checked
sort row uses `Array[SortRecord^{epoch}]^{epoch}` and region-owned
`SortRecord` objects, so it is not a heap-array fallback. At 1M sort records,
`checked-epoch-stream` is `230.000 ms` and `checked-epoch-scoped` is
`230.799 ms` versus heap `235.554 ms` and improved SafeZone `233.068 ms`.
...
SELECT/AGGREGATE/JOIN: `19.318/36.016/20.082 ms` versus `gc-heap`
`26.996/52.754/31.281 ms` and improved SafeZone `23.339/44.737/24.136 ms`.
Heap median timed GC is `6.355/13.209/9.672 ms`; direct checked epoch removes
that GC. The scoped page-token SELECT row is slightly faster (`19.010 ms`) but
only covers SELECT/filter/project, while direct epoch covers the whole
Dataflow operator family. The old generic `checked-epoch-fold` aggregate row
is still a negative control at `93.491 ms`.
...
Outcome labels now distinguish the kind of win:

- **Representative throughput win** means elapsed time improves enough to
  support the final story.
- **Modest throughput win** means elapsed improves, but the margin or
  provenance is not strong enough for a representative claim.
- **RSS win** means resident memory improves materially even if elapsed is tied
  or slightly worse.
- **Fixed-memory / tail win** means heap is competitive when uncapped but
  regions improve capped-heap behavior, max-GC tails, or deadline misses.
- **Speed-gated** means correctness and some GC/RSS benefit exist, but CPU
  overhead is too high for public performance claims.
- **Ceiling/negative** means no material elapsed, RSS, GC, or latency benefit
  was measured at the current scale.

## Source Files

| Area | Source |
|---|---|
| 2026-05-06 staged comprehensive headline sweep | `evidence/COMPREHENSIVE_SWEEP_2026_05_06.md` |
...
| 2026-05-01 UnsafeZone-HP core/prior headline sweep | `evidence/HEADLINE_UNSAFEZONE_CORE_PRIOR_2026_05_01.md` |
| 2026-05-01 UnsafeZone-HP stream headline sweep | `evidence/HEADLINE_UNSAFEZONE_STREAMS_2026_05_01.md` |
...
| Common Crawl-like object-heavy stream expansion | `evidence/COMMON_CRAWL_LIKE_MATRIX.md` |
| Phase 6 Wikimedia pageview/clickstream detector | `evidence/WIKIMEDIA_REGION_MATRIX.md` |
| Phase 6 Linear Road methodology detector | `evidence/LINEAR_ROAD_REGION_MATRIX.md` |
...
| CPU profiling scaffold | `docs/CPU_PROFILE_REPORT.md` |
| Literature benchmark comparison contract | `docs/LITERATURE_BENCHMARK_CONTRACT.md` |
| Current roadmap status | `docs/ROADMAP.md` |
| Handoff / caveats | `docs/HANDOFF.md` |

## Latest Staged Sweep Snapshot

Source: `evidence/COMPREHENSIVE_SWEEP_2026_05_06.md`.

The latest staged headline sweep used run IDs
`2026-05-05-comprehensive-headline` and
`2026-05-05-comprehensive-headline-cont`. Competitive rows skip
`current-safezone`; the continuation reran SafeZone-cost with
`current-default` excluded after the first run was stopped on that unwanted
row. Core runtime/topology headline rows were not rerun in this pass.

| Area | Latest result | Meaning |
|---|---|---|
| Dataflow SELECT | checked scoped page-token `18.458 ms` vs heap `29.347 ms` | reusable checked page-token remains the strongest simple map/filter operator |
| Dataflow AGGREGATE | checked exact-array aggregate `40.098 ms`; true `EpochFold` `92.923 ms` | current checked aggregate wins, reusable fold remains speed-gated |
...
| Phase 6: literature-aligned methodology evidence | Started | Validated methodology medians with caveats; real-input search in progress | Broom-style dataflow including checked SELECT/AGGREGATE/JOIN modes, StreamFlex-style latency/throughput with direct checked epoch, Yak-style control/data plus grouped sort, top-word/filter, GraphChi-style subintervals, and runtime promotion proxy, Stancu-style transaction accounting with direct checked epoch, NEXMark-lite Q0/Q1/Q2/Q5/Q8 stream queries, Beam-default NEXMark-profile rows including expanded Q3/Q4/Q9/Q11, generated and real/preloaded Common Crawl/Wikimedia/Linear Road stream detectors, Yahoo-style ad stream, RIoTBench-style IoT stream, real GH Archive and LogHub probes, Common Crawl-like q1/q2 fast-allocation-counter follow-up, DSPBench source/data triage plus Spike Detection and Fraud Detection q0/q1/q2 local matrices, and the Scala Native win-envelope synthesis |
...
| Cheap checked operator families | Started, first reusable APIs validated | Compile/tests plus focused 1M-shape 3-run, direct epoch rows, StreamFlex transaction rows, RSS rows, and staged headline sweep | `PageTokenMapFilter`, `RegionList`, direct `RiftRegion.epoch`, `EpochBuffer`, and `TransactionRegion` are now real reusable APIs. Latest staged sweep rows are positive for SELECT and cheap append/drain: SELECT scoped page-token `18.458 ms` vs heap `29.347 ms`; scoped checked EpochBuffer `26.461 ms` and scoped checked page-token `26.883 ms` vs heap `36.944 ms`. `RegionList` remains the linked-topology reusable API from the previous focused gate. Direct checked epoch now supersedes `TransactionRegion` on StreamFlex-shaped throughput: at 1M, scoped direct epoch is `163.339 ms` vs heap `218.582 ms`, improved SafeZone `208.653 ms`, trusted Streaming `182.246 ms`, and scoped checked TransactionRegion `205.929 ms`. `EpochFold` is implemented and correct but failed the latest speed gate at `92.923 ms`; the `40.098 ms` aggregate row remains exact-array checked aggregate evidence, not true `EpochFold` evidence. |
| StreamFlex direct checked epoch integration | Representative checked win | Compile, smoke, 200k throughput/latency, and 1M throughput rows | `StreamFlexRegionMatrix` now accepts `rift-checked-direct-epoch` and `rift-checked-safezone-direct-epoch`. Checksums match. The direct epoch path uses one checked epoch block and ordinary linked Scala objects for all four pipeline stages. At 200k, scoped direct epoch is fastest for throughput (`32.029 ms`) and latency (`9.636 ms`); at 1M throughput, scoped direct epoch is `163.339 ms` versus heap `218.582 ms`, improved SafeZone `208.653 ms`, trusted Streaming `182.246 ms`, and scoped checked TransactionRegion `205.929 ms`. |
| Stancu direct checked epoch integration | Representative checked win | Compile, smoke, 200k, and 1M rows | `StancuRegionMatrix` now accepts `rift-checked-direct-epoch` and `rift-checked-safezone-direct-epoch`. Checksums match. The direct epoch path uses one checked epoch block per transaction batch; ordinary order/line objects live in the epoch, while stock/revenue/customer arrays stay on heap. At 1M, scoped direct epoch is `160.198 ms`, checked stream epoch is `174.137 ms`, improved SafeZone is `186.122 ms`, trusted Streaming is `219.668 ms`, and heap is `225.798 ms`. |
| StreamFlex checked TransactionRegion integration | Superseded for this shape; still useful as multi-list control | Compile, compiler/runtime safety, smoke, 200k/1M-shape throughput, and latency rows | `StreamFlexRegionMatrix` accepts `rift-checked-transaction-region` and `rift-checked-safezone-transaction-region` in addition to EpochBuffer controls. `TransactionRegion` reduces checked Rift opens/closes compared with stacked EpochBuffer and keeps one child region per transaction, but the direct checked epoch row is now faster on this benchmark because the stage objects can be consumed as ordinary epoch-local linked lists. |
| Final component selection and default-run pruning | Started | Syntax-validated runner/script update, dirty-worktree smoke validation, and clean headline medians | `docs/FINAL_COMPONENT_SELECTION.md` now defines public candidates, internal controls, and gated/rejected operators. The parent performance runner plus the main sandbox matrix scripts now exclude current SafeZone and rootless/unsafe controls by default. Set `RIFT_EVAL_INCLUDE_CONTROLS=1`, `RIFT_BENCH_INCLUDE_CONTROLS=1`, or explicit `*_MODES` variables to reproduce lower-bound/control rows. Run `2026-05-06-final-selection-smoke` validated the default path. Clean run `2026-05-06-final-selection-headline` then completed `preflight core prior checked streams reml` with `include_controls=0`; see `evidence/FINAL_SELECTION_HEADLINE_2026_05_06.md`. Key rows: checked scoped page-token q1/q2 Common Crawl-shaped `3860.248/3895.711 ms` vs heap `5619.896/5429.530 ms`; checked page-token GH Archive-shaped q1/q2 `262.139/261.762 ms` vs heap `293.716/279.743 ms`; checked ListOfLists builder `6053.235 ms` vs heap `15820.172 ms`; checked stream ReML-shaped `msort-r` `103.878 ms` vs heap `127.936 ms`. |
...
| Phase 3 | Reran same-layout runtime matrices, then added an UnsafeZone-HP substrate control. | Older Rift allocator/runtime wins on GCBench and linked structures are weakened by the clean 2026-05-01 reruns. Improved SafeZone is already strong, and UnsafeZone-HP usually beats current Rift HPZone on linked/prior-work rows. Rift HPZone still wins flat ListOfLists. |
...
| Yak control/data split | Wordcount, graphstep, grouped sort, top-word/filter, GraphChi-style subintervals, promotion/escape epoch workloads, and SNAP `graphreal` replay with durable heap control state | No-escape pressure rerun shows raw Rift and `yak-runtime` both beat heap and remove measured heap GC. Grouped sort now has a checked direct-epoch array row: `checked-epoch-stream` `230.000 ms` and `checked-epoch-scoped` `230.799 ms` versus heap `235.554 ms` and improved SafeZone `233.068 ms`; the older trusted HPZone row remains `227.393 ms`. Top-word/filter and GraphChi-style rows are now also covered by the reusable `RiftRegion.epoch` API: at 10M logical objects, `checked-epoch-scoped` is fastest among measured rows for wordcount (`164.825 ms`), graphstep (`181.341 ms`), topword (`239.507 ms`), and graphchi (`195.816 ms`), while heap spends `46-56 ms` in timed GC. The corrected runtime-promotion proxy records 10M barrier checks, 10k remembered refs, and 20k promoted objects, but Yak-runtime is slower than heap: `513.465 ms` vs `424.768 ms`. The real LiveJournal `graphreal` topology row is now the best Yak-shaped signal: the apples-to-apples API-backed 50M rerun has `checked-epoch-scoped` `1055.958 ms` and `checked-epoch-stream` `1101.001 ms` versus `gc-heap` `1604.811 ms` with `288.801 ms` timed GC, and region RSS around `1.53 GB` versus heap `2.76 GB`. Whole-run checked scoped is faster in the older topology table but high-RSS; page-token checked remains a win but is not the right Yak topology. Reusable `EpochBuffer` integrates with this path but is slower than direct checked epoch. | Not distributed Yak; sort/topword/graphchi/graphreal are local Hyracks/Hadoop/GraphChi-shaped methodology probes, and promotion is a memory-API-level proxy with object-specific `RuntimePromoter`, not real JVM field barriers, stack scanning, STW coordination, or generic object movement. Sort is now covered by checked region-captured arrays but remains CPU-bound. |
...
| NEXMark-lite / Beam-default streaming | Deterministic auction stream with Q0 passthrough, Q1 currency conversion, Q2 selection, Q3 incremental join/filter, Q4 category average, Q5 hot-items window, Q8 new-user/new-auction window join, Q9 winning-bid core, and Q11 session windows | Local 1M medians show Q1 checked `374.767 ms` vs heap `384.595 ms`, Q2 checked `287.808 ms` vs heap `297.053 ms`, generic Q8 checked `291.832 ms` vs heap `322.210 ms`, and Q5 checked fold API `419.855 ms` vs heap `470.288 ms`. Beam-default Q0 is a trusted-runtime win, Q1 remains promising, Q3 is the best new checked row (`287.169 ms` checked vs heap `304.190 ms` and improved SafeZone `293.586 ms`), Q8 is a modest checked win, and Q11 is a trusted HPZone win. Q2/Q4/Q9 are ceiling or improved-SafeZone-favored controls. | Local methodology benchmark and Beam-default generated profile, not exact Apache Beam runner evidence; Q5 fold API is generated-local, not Beam-default. |
| Common Crawl WET tokenization | Generated and real preloaded page/line/token streams with bucket-owned ordinary Scala records | Corrected generated 100k default tokenization stresses heap (`149.149 ms` GC, `408305664` RSS bytes), but improved SafeZone is fastest in that older row (`381.006 ms` vs Streaming `403.935 ms`). After the Rift fast-allocation counter cleanup, generated 1M q1/q2 become trusted-Rift wins: q1 HPZone `4386.590 ms` vs heap `5466.535 ms` and improved-32k `4608.641 ms`; q2 Streaming `4164.288 ms` vs heap `5267.784 ms` and improved-32k `4425.273 ms`. Checked q1/q2 validate safe record placement and beat heap in the RSS-complete rerun, but miss improved-SafeZone/trusted gates. Real WET preloaded rows are heap-fastest with median-zero timed GC: 10k pages heap `12.079 ms` vs Streaming `15.651 ms`; 50k requested / 21425 actual pages heap `26.452 ms` vs Streaming `33.103 ms`. | Real preloaded WET does not clear the case-study gate; generated q1/q2 are trusted runtime wins; checked q1/q2 are checked-overhead evidence. |
...
| Checked window-fold operator | Additive per-key stream-window aggregate with child-bucket ordinary Scala records and parent-owned primitive metadata | The 2026-06-01 focused rerun after redundant `putFoldInBucket` open-check removal now passes narrowly: `97.321 ms` checked vs `99.302 ms` heap, same checksum, checked GC `0.000 ms`, and RSS `40386560` vs heap `75005952`. | Focused framework evidence only; Common Crawl WET, NEXMark Q5, and DEBS fold integrations still need their own smokes/L1/L2 before becoming application claims. |
...
| q5 hot items | 470.288 | fold API `419.855` | n/a | n/a | The older generic checked path was not a win; the 2026-06-01 `StreamWindowFold` API path passes generated-local 1M L2 with zero checked GC and lower RSS. |
| q8 window join | 322.210 | 291.832 | 305.338 | 305.410 | Strongest checked NEXMark-lite row so far; GC drops from `27.148 ms` to `7.413 ms`. |
...
Beam-default generated profile follow-up:

| Query | Scale | Heap ms | Improved SafeZone ms | Best Rift ms | Interpretation |
|---|---:|---:|---:|---:|---|
| q1 currency conversion | 1M | 579.038 | 561.787 | HPZone `538.451`, checked `557.251` | Best new positive profile row; generated Beam-default settings, not Beam runner evidence. |
| q2 selection | 1M | 375.514 | 382.855 | Streaming `377.816` | Lower GC but elapsed near-tie. |
| q8 window join | 1M | 331.599 | 326.569 | checked `315.545` | Modest checked win, below the case-study margin. |
...
| Scale | Mode | Adds/removes | Closed buckets | Samples | Top scan entries | Top scan ms | Clean median ms |
|---|---|---:|---:|---:|---:|---:|---:|
| 100k | heap | 100000 / 100000 | 4 | 13 | 851968 | 4.791 | diagnostic single-run only |
| 100k | rift-checked | 100000 / 100000 | 4 | 13 | 851968 | 4.803 | diagnostic single-run only |
| 1M | heap | 1000000 / 1000000 | 40 | 123 | 8060928 | 45.340 | 361.882 |
| 1M | rift-checked | 1000000 / 1000000 | 40 | 123 | 8060928 | 46.823 | 393.415 |

The diagnostic rows are not headline timings. They show Q5 performs the same
window operations in every mode and that top-auction scans are a shared cost,
not a Rift-specific explanation for the older generic checked loss. The later
`StreamWindowFold` API gate supersedes that generic checked path for Q5.

### Common Crawl WET-Shaped Detector, 2026-04-30

Source: `evidence/COMMON_CRAWL_WET_MATRIX.md`

This is a generated WET-shaped object stream, not a real Common Crawl artifact
run. It deliberately removes file I/O and decompression to expose allocation
pressure from page, line, and token objects.

100k default-bucket tokenization:
...
| Requested pages | Actual pages | Heap ms | Improved SafeZone ms | Best Rift ms | GC ms | Interpretation |
|---:|---:|---:|---:|---:|---:|---|
| 10000 | 10000 | 12.079 | 16.093 | Streaming `15.651` | Median `0.000` | Heap fastest; not GC-bound after preloading. |
| 50000 | 21425 | 26.452 | 30.730 | HPZone `32.809` | Median `0.000` | Heap fastest; actual page count is below requested count, so treat as a larger small-input row. |
...
- The small-bucket control is the important caveat: if the logical program
  ...
  much of the gap.
- The first real WET shard does not help the case: heap is fastest and timed
  GC is zero after preloading.
- Revisit Common Crawl only if a different real WET/WAT input shows allocation
  pressure or a checked page/token append API passes a focused gate.

### Common Crawl-Like Fast-Allocation And Checked Follow-Up, 2026-05-02

Source: `evidence/COMMON_CRAWL_LIKE_MATRIX.md`

This follow-up uses the generated WET-shaped q1/q2 workload after removing
default per-allocation global allocated-byte atomics from the Rift fast path.
It then adds an opt-in `rift-checked` mode using the reusable checked
`StreamAppendWindow` cursor API for page/line/token records.

Trusted q1/q2 1M medians after fast-allocation counter cleanup:

| Query | Heap | Improved 32K | UnsafeZone-HP | Best trusted Rift | Interpretation |
|---|---:|---:|---:|---:|---|
| q1 tokenization | `5466.535 ms`; GC `1580.847 ms` | `4608.641 ms` | `4640.245 ms` | HPZone `4386.590 ms`; GC `20.298 ms` | Trusted Rift beats heap, improved SafeZone-32k, and UnsafeZone-HP. |
| q2 domain window | `5267.784 ms`; GC `1561.851 ms` | `4425.273 ms` | `4437.924 ms` | Streaming `4164.288 ms`; GC `20.044 ms` | Trusted Rift beats heap, improved SafeZone-32k, and UnsafeZone-HP. |

Checked q1/q2 1M RSS-complete medians:

| Query | Heap | Improved 32K | Trusted Rift | Rift checked | Interpretation |
|---|---:|---:|---:|---:|---|
| q1 tokenization | `5670.270 ms`; GC `1745.758 ms` | `4644.747 ms` | HPZone `4403.007 ms` | `5088.712 ms`; GC `20.942 ms` | Checked beats heap and matches output, but misses improved-SafeZone/trusted gate. |
| q2 domain window | `5342.373 ms`; GC `1605.929 ms` | `4444.954 ms` | HPZone `4258.549 ms` | `5061.479 ms`; GC `24.889 ms` | Checked beats heap modestly and matches output, but misses improved-SafeZone/trusted gate. |

Interpretation:

- The trusted q1/q2 rows are the strongest current GC-heavy stream-object
  runtime/backend evidence.
- The checked q1/q2 rows validate ordinary records in checked child-bucket
  regions at application scale: `137000000` records, matching checksums/output
  counts, and heap GC reduced from about `1.6-1.7 s` to about `20-25 ms`.
- The checked rows are not application speed evidence yet. Measured Rift op
  time is only about `11 ms`, so the gap is checked `StreamAppendWindow`
  append/cursor/container overhead in hot loops, not allocator close/reclaim.
- RSS remains a caveat: heap RSS is lower than the SafeZone/Rift-family rows
  in the 1M runs.

### Wikimedia Generated Pageview/Clickstream Detector, 2026-05-01

Source: `evidence/WIKIMEDIA_REGION_MATRIX.md`

This is a generated TSV-shaped object stream, not a real Wikimedia dump run.
It tests pageview, project/article count-update, and clickstream edge-update
objects in bucket lifetimes.

1M medians:

| Query | Heap ms | Improved SafeZone ms | Best Rift ms | Heap GC ms | Best Rift GC ms | Interpretation |
|---|---:|---:|---:|---:|---:|---|
| q0 pageviews | 57.760 | 59.145 | Streaming `59.182` | 12.242 | 0.000 | Heap fastest; not a Rift case. |
| q1 counts | 144.279 | 144.272 | HPZone `146.305` | 32.902 | 2.303 | Near-tie with improved SafeZone/heap ahead. |
| q2 clickstream | 159.746 | 147.936 | HPZone `147.163` | 35.238 | 2.206 | Promising at 1M, but not a 10% win over improved SafeZone. |

10M Q2 single-run scale check:

| Mode | Median ms | GC ms | Rift op ms | Region objects | Peak RSS bytes | Interpretation |
|---|---:|---:|---:|---:|---:|---|
| heap | 1459.438 | 345.918 | 0.000 | 0 | 12402688 | Lowest elapsed in this single run, high GC. |
| safezone-improved | 1473.088 | 29.698 | 0.000 | 0 | 13778944 | Low GC, near heap. |
| rift-hp | 1462.015 | 21.341 | 2.511 | 20000000 | 13746176 | Low GC but elapsed near-tie, not a strong win. |
| rift-streaming | 1464.663 | 21.189 | 2.830 | 20000000 | 13729792 | Same conclusion as HPZone. |

Interpretation:

- Generated Wikimedia Q2 confirms a structured-lifetime stream shape can cut
  measured GC substantially.
- The elapsed result is not strong enough for a case-study claim: the 10M Q2
  scale check is a near-tie, and Q0/Q1 do not win.

Real enwiki clickstream preloaded follow-up:

| Scale | Heap ms | Improved SafeZone ms | HPZone ms | Streaming ms | GC ms | Interpretation |
|---:|---:|---:|---:|---:|---:|---|
| 100k | 11.534 | 14.835 | 15.202 | 15.293 | Median `0.000` | Heap fastest; not GC-bound. |
| 1M | 126.800 | 149.062 | 158.831 | 157.449 | Median `0.000`; heap had one `67.236 ms` GC outlier | Heap fastest; preloaded real input weakens the generated Q2 story. |

- Revisit Wikimedia only after a reusable operator changes allocation cost, or
  when measuring file-backed/parser behavior as a separate claim.

### Linear Road Generated Methodology Detector, 2026-05-01

Source: `evidence/LINEAR_ROAD_REGION_MATRIX.md`

This is a generated Linear Road-shaped stream, not an exact Linear Road
artifact run. It tests position reports, toll-output objects, and
accident/congestion candidate objects with durable vehicle/segment metadata
kept in heap primitive arrays.

1M medians:

| Query | Heap ms | Improved SafeZone ms | Best Rift ms | Heap GC ms | Best Rift GC ms | Interpretation |
|---|---:|---:|---:|---:|---:|---|
| q0 reports | 93.798 | 92.615 | HPZone `94.908` | 10.413 | 0.000 | Near-tie; improved SafeZone fastest. |
| q1 tolls | 170.464 | 196.138 | HPZone `191.896` | 24.819 | 0.000 | Rift beats improved SafeZone but not heap. |
| q2 accidents | 194.520 | 215.808 | HPZone `203.793` | 27.604 | 0.000 | Rift beats improved SafeZone but not heap. |

Interpretation:

- Linear Road-shaped generated streams remove measured GC in Rift, but heap is
  still fastest for q1/q2 at 1M.
- Region modes do not improve q1/q2 RSS at the default bucket size; live
  bucket-retained position/output objects dominate.

Official Data Driver preloaded follow-up:

| Query | Heap ms | Improved SafeZone ms | Best Rift ms | GC ms | Interpretation |
|---|---:|---:|---:|---:|---|
| q0 reports | 88.750 | 103.086 | Streaming `99.769` | Median `0.000`; heap had one `52.436 ms` GC outlier | Heap fastest; region modes lower RSS only. |
| q1 tolls | 162.668 | 179.205 | HPZone `180.277` | Median `0.000`; heap had one `45.194 ms` GC outlier | Heap fastest; weak collection-time ceiling. |
| q2 accidents | 167.811 | 199.116 | Streaming `198.863` | Median `0.000`; heap had one `59.832 ms` GC outlier | Heap fastest; weak collection-time ceiling. |

- This is useful negative evidence for the stream-benchmark ladder. Do not tune
  Linear Road-specific code into a benchmark claim.
...
Focused cheap checked append-window benchmark:

Sources:

- `evidence/CHECKED_APPEND_WINDOW_MATRIX.md`
- `sandbox/src/main/scala-next/CheckedAppendWindowMatrix.scala`
- `sandbox/run_checked_append_window_matrix.sh`

100k median, 100000 events, 25000 events per bucket, 8 live buckets:

| Mode | Median elapsed ms | Median GC ms | Median Rift op ms | Region objects | Opens/closes/resets | Peak RSS bytes |
|---|---:|---:|---:|---:|---:|---:|
| heap | 2.782 | 0.000 | 0.000 | 0 | 0 / 0 / 0 | 21282816 |
| rift-checked | 3.370 | 0.000 | 0.008 | 100000 | 5 / 5 / 0 | 15728640 |
| rift-trusted-hp | 4.087 | 0.000 | 0.006 | 100000 | 4 / 4 / 0 | 15646720 |
| rift-trusted-streaming | 4.211 | 0.000 | 0.017 | 100000 | 4 / 4 / 0 | 15712256 |

1M median, same stream-window shape:

| Mode | Median elapsed ms | Median GC ms | Median Rift op ms | Region objects | Opens/closes/resets | Peak RSS bytes |
|---|---:|---:|---:|---:|---:|---:|
| heap | 35.513 | 11.149 | 0.000 | 0 | 0 / 0 / 0 | 74989568 |
| rift-checked | 32.261 | 0.000 | 0.074 | 1000000 | 41 / 41 / 0 | 47497216 |
| rift-trusted-hp | 41.641 | 0.000 | 0.093 | 1000000 | 40 / 40 / 0 | 47349760 |
| rift-trusted-streaming | 41.859 | 0.000 | 0.088 | 1000000 | 40 / 40 / 0 | 47480832 |

Reusable `StreamAppendWindow` API follow-up, same 1M workload:

| Mode | Median elapsed ms | Median GC ms | Median Rift op ms | Region objects | Opens/closes/resets | Peak RSS bytes |
|---|---:|---:|---:|---:|---:|---:|
| heap | 37.424 | 11.596 | 0.000 | 0 | 0 / 0 / 0 | 75071488 |
...
| Mode | Median elapsed ms | Median GC ms | Median Rift op ms | Region objects | Opens/closes/resets | Peak RSS bytes |
|---|---:|---:|---:|---:|---:|---:|
| heap | 38.559 | 11.581 | 0.000 | 0 | 0 / 0 / 0 | 74989568 |
| rift-checked | 33.172 | 0.000 | 0.083 | 1000000 | 41 / 41 / 0 | 47497216 |
| rift-checked-api | 39.372 | 0.000 | 0.084 | 1000000 | 41 / 41 / 0 | 47480832 |

Cursor close follow-up, same 1M workload:

| Mode | Median elapsed ms | Median GC ms | Median Rift op ms | Region objects | Opens/closes/resets | Peak RSS bytes |
|---|---:|---:|---:|---:|---:|---:|
| heap | 35.705 | 11.095 | 0.000 | 0 | 0 / 0 / 0 | 75022336 |
| rift-checked | 32.367 | 0.000 | 0.077 | 1000000 | 41 / 41 / 0 | 47529984 |
| rift-checked-api | 37.705 | 0.000 | 0.073 | 1000000 | 41 / 41 / 0 | 47529984 |
| rift-checked-api-cursor | 34.708 | 0.000 | 0.074 | 1000000 | 41 / 41 / 0 | 47529984 |

Interpretation:

- This is the cheap checked operator contrast to TableRank. It deliberately
  avoids ranking, table lookup, top-k extraction, formatting, and DEBS parsing.
- At 100k, heap is faster and measured GC is zero. The only clear Rift benefit
  is lower RSS. This is below the allocation-pressure threshold.
- At 1M, checked Rift is about `9.2%` faster than heap, eliminates
  `11.149 ms` of measured GC collection time, and lowers peak RSS by about
  `27.5 MB`.
- Rift operation time is tiny (`0.074 ms`) for one million ordinary Scala
  region objects and 41 structured region closes.
- The trusted per-bucket HPZone/Streaming rows are slower than heap in this
  harness. Treat the result as a checked child-bucket/operator-shape win, not
  as a generic trusted-region win.
- This is the current concrete evidence for the "SN win envelope" direction:
  cheap append/window stream operators can win when object volume is large
  enough, while maintenance-heavy checked ranking still needs container CPU
  work before DEBS Q1 integration.
- The per-entry `StreamAppendWindow` API is still not ready for application
  ...
  `66.023 ms`, and benchmark-side cached bucket/region use improves it to
  `39.372 ms`, but the per-entry close shape still misses the strict gate.
...
- Opt-in `CHECKED_APPEND_API_DIAG=1` counters after bucket/region caching show
  ...
  close entries, and final live length `0`. The remaining per-entry issue was
  close callback/link traversal shape, which cursor close targets.

### Checked Window Fold Matrix, 2026-04-30

Source: `evidence/CHECKED_WINDOW_FOLD_MATRIX.md`

The additive fold API was added to test whether a reusable checked
window-aggregate primitive can carry the Common Crawl/NEXMark Q5 direction.
It uses ordinary Scala records in child bucket regions and parent-owned
primitive aggregate tables.

Historical 1M 3-run medians before the redundant open-check removal:

| Mode | Median ms | GC ms | Rift op ms | Region objects | Peak RSS bytes | Interpretation |
|---|---:|---:|---:|---:|---:|---|
| heap | 103.244 | 11.910 | 0.000 | 0 | 75022336 | fair heap control |
| rift-checked | 118.726 | 0.000 | 0.175 | 1000004 | 40402944 | lower GC/RSS, failed speed gate before check-removal fix |
| rift-trusted-hp | 104.919 | 0.000 | 0.117 | 1000000 | 46841856 | close to heap, not a win |
| rift-trusted-streaming | 106.088 | 0.000 | 0.120 | 1000000 | 46956544 | close to heap, not a win |

Superseding 2026-06-01 focused rerun:

| Mode | Median ms | GC ms | Rift op ms | Region objects | Peak RSS bytes | Interpretation |
|---|---:|---:|---:|---:|---:|---|
| heap | 99.302 | 10.518 | 0.000 | 0 | 75005952 | fair heap control |
| rift-checked | 97.321 | 0.000 | 0.148 | 1000004 | 40386560 | focused checked win; application gates still pending |

Interpretation:

- The focused gate now passes narrowly after redundant open-check removal.
- The memory result remains useful: checked removes measured GC, keeps Rift op
  time below `1 ms`, and cuts RSS by about `34.6 MB`.
- Common Crawl WET fold-backed integration should still wait for its own
  correctness and L1/L2 gates; NEXMark Q5 now has a generated-local fold API
  L2 gate.

DEBS Q1 event-window cursor integration:
...
- Checked Q1 event-window entries now use
  `RiftRegion.StreamAppendWindow[RouteEvent]`.
- `RouteEvent` is an ordinary Scala class extending
  `RiftRegion.StreamAppendNode`, allocated in the current checked child bucket.
- Expired Q1 event buckets are consumed through
  `closeAppendWindowBucketsBeforeWithCursor`; end-of-run cleanup uses
  `closeAllAppendWindowBucketsWithCursor`.
- Q1 ranking/TableRank and Q2 ranking were not changed.
...
- Q1 checked-processing sample output matched heap.
- Q1 checked-processing 100k output matched heap.
...
- This is the first DEBS application use of the passing
  `StreamAppendWindow` cursor-close shape and now has bounded 1M median-backed
  control evidence.
- It keeps heap/Rift logical behavior aligned and changes allocation/lifetime
  placement for checked Q1 event-window storage.
- Treat the rows as bounded application-path evidence, not final full-DEBS
  proof. The 1M median shows much lower RSS and low Rift bookkeeping while Q1
  process time remains higher than heap.
...
- Runtime-only Rift wins on GCBench and linked ListOfLists are no longer stable
  headline claims after the clean 2026-05-01 reruns. Current Rift HPZone still
  wins flat ListOfLists, but improved SafeZone and UnsafeZone-HP are stronger
  on linked/core-prior rows.
- Improved SafeZone is a serious baseline and cannot be omitted. UnsafeZone-HP
  now shows that SafeZone internals with root registration disabled are often
  faster than the current Rift backend across core/prior and stream probes, but
  that mode is unsafe and benchmark-only.
...
- The new `SN_WIN_ENVELOPE.md` synthesis is the current guide for choosing the
  next Scala Native work: Rift wins where allocation-heavy stream data has a
  structured lifetime and the operator does not spend most of its time in
  ranking/table maintenance or I/O.
- `CheckedAppendWindowMatrix` is the new positive focused result for that
  envelope. At 1M events, checked Rift is `32.261 ms` versus heap `35.513 ms`,
...
- `NEXMARK_REGION_MATRIX.md` is now the first broader non-DEBS ladder for that
  ...
  `287.808 ms` versus heap `297.053 ms`; Q8 window join gives checked
  `291.832 ms` versus heap `322.210 ms`; and the later Q5 fold API gate gives
  checked `419.855 ms` versus heap `470.288 ms`. Q0 passthrough is a near tie.
...
- The Q5 diagnostic follow-up confirms identical window operation counts and
  about `45 ms` of top-auction scans in every 1M mode. The older generic
  checked path remained slower (`393.415 ms`) than heap (`361.882 ms`), but
  the later `StreamWindowFold` API path fixes the Q5 gate for generated-local
  evidence.
...
- The next checked-overhead split is now explicit. Region close/open cost is
  ...
  `8.906 ms`). Its first Common Crawl-shaped q2 application gate also missed:
  at 100k generated pages, checked SafeZone-backed count-by-key is
  `450.289 ms` versus existing checked SafeZone-backed page-token
  `406.413 ms`, with matching output. Treat it as reusable append-time
  aggregate evidence and a gated application control, not a broad
  checked-overhead solution.
...
- The file-backed GH Archive q2 follow-up makes the result stronger. At 100k
  real events, uncapped heap is `3995.632 ms`, median GC `158.277 ms`, RSS
  `1218428928`; `2G` and `1400M` heap caps complete, while `1G` fails with
  signal 11 at `1077067776` bytes RSS. Improved SafeZone is `3934.094 ms`,
  trusted Streaming is `3906.291 ms`, and checked SafeZone-backed page-token is
  `3921.127 ms`, all with about `672-674 MB` RSS and matching output count
...
- A larger LogHub / LogPAI HDFS v1 system-log input is now downloaded and
  measured. The extracted file has `11175629` lines and lives at
  `/Users/siyaoliu/rift/cache/benchmark-data/loghub/HDFS_1/HDFS.log`; archive
  SHA-256 is
  `6ca6c5bc2671c66afecee9369a2fdac606bf33997a2494ac66aa411fe3e95169`.
  At 1M, q1 heap is `8059.155 ms`, median/max GC `139.906/191.623 ms`, RSS
  `408551424`; checked scoped page-token is `7923.079 ms`, zero timed GC, RSS
  `356319232`, and improved SafeZone-32k is `7919.352 ms`. q2 is the strongest
  checked LogHub row so far: heap `8227.369 ms`, median/max GC
  `92.659/104.790 ms`, RSS `408666112`; checked scoped page-token
  `7871.856 ms`, zero timed GC, RSS `394969088`; trusted Streaming
  `7871.713 ms`, zero timed GC, RSS `356073472`. q3 cuts RSS but checked
  elapsed loses slightly (`8506.708 ms` vs heap `8460.928 ms`). Interpretation:
  HDFS is a real-input modest checked throughput/RSS/GC-tail win, especially
  q2, but not a flagship GC-heavy case because parser/token/template CPU
  dominates and heap GC is still only about 1% of elapsed.
- The real-input benchmark search now has its own ledger:
  ...
  Theodolite source has also been cloned and inspected at
  `/Users/siyaoliu/rift/cache/benchmark-data/theodolite/source`; UC2/UC4 are
  relevant industrial-IoT stream shapes, but the checked source provides
  simulated active-power load generation rather than a static real input
  dataset, so Theodolite remains methodology evidence unless paired with a
  real industrial-energy trace.
