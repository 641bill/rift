...
| Phase 6c: ReML / MLKit lineage comparison | Started | Paper-reported table plus local scaffold; no exact medians yet | Elsman 2023 Figure 9 transcribed as paper-reported/not-rerun evidence; `ReMLRegionMatrix` Tier 1 Scala Native-shaped ports for `fib37`, `tak`, `mandel`, `msort`, `msort-r`, `life`, `fft`, and `ratio`; ReML-style durable/static generic heap-retention probe is active and passing; public MLKit source provenance has started from the ignored clone at `cache/reml/mlkit`, with Figure 9-style sources found and tags `v4.7.4`/`v4.7.5`/`v4.7.6` identified; exact local `mlkit`/`mlton` reproduction remains open |
