...
| Phase 6c ReML / MLKit lineage comparison | `evidence/REML_COMPARISON_MATRIX.md`; `docs/REML_COMPARISON_PLAN.md` |
...
| Phase 6c: ReML / MLKit lineage comparison | Started | Paper-reported table plus local scaffold; no headline medians yet | Elsman 2023 Figure 9 transcribed as paper-reported/not-rerun evidence; `ReMLRegionMatrix` Tier 1 Scala Native-shaped ports for `fib37`, `tak`, `mandel`, `msort`, `msort-r`, `life`, `fft`, and `ratio`; ReML-style durable/static generic heap-retention probe is active and passing; exact artifact reproduction open |
