...
| Phase 6 | Built methodology harnesses for Broom/StreamFlex/Yak/Stancu comparison axes, then added NEXMark-lite, Beam-default NEXMark-profile rows, Common Crawl WET-shaped, Wikimedia pageview/clickstream, Linear Road-shaped, Yahoo-style ad-stream, and RIoTBench-style IoT stream-processing probes with generated and real/preloaded inputs where available. | These support the broader research story but are not exact reproductions of closed or unavailable artifacts. NEXMark-lite Q1/Q2 show modest 1M elapsed wins and lower GC, generic Q8 is a checked join-window win, the reusable Q8 `StreamJoinWindow` API is lower-GC/lower-RSS but slower than a fair specialized heap control, and Q5 remains checked-container/window-overhead evidence rather than a window-aggregate win. Beam-default Q0/Q1/Q3/Q8/Q11 are the useful new profile rows, with Q3 the best checked expanded-query row. Real enwiki clickstream, real Common Crawl WET, and official Linear Road preloaded rows are ceiling results. Yahoo Q2 cuts GC but does not beat heap at 1M. RIoTBench-style q1 stresses heap GC and Rift beats heap, but improved SafeZone remains faster. |
