...
| Phase 7: capture-checked safe API | Started | Compiler-probe, runtime-smoke, focused checked-container benchmark, checked dataflow evidence, and DEBS-shaped checked probes | 69 targeted checked-API compiler probes, checked child-window, child-bucket, stream-bucket-arena, stream-window-rank, and window-rank auto-cleanup runtime probes, `CheckedRegionBufferMatrix`, `CheckedRegionPriorityQueueMatrix`, `CheckedRegionIndexedPriorityQueueMatrix`, `CheckedStreamWindowRankMatrix`, Dataflow SELECT/AGGREGATE/JOIN `rift-checked`, Q1 checked-output, Q1 checked-processing, Q2 checked-processing, checked RunBoth `rift-checked` medians, plus Phase 4 safety finding |
...
| Phase 7 | Added checked `scoped`/`streaming` API probes using Scala capture checking, then moved from buffers to ranking and stream-window ranking containers. | Source-level safety evidence is started. The stream-window rank matrix validates the general bucket-region object pattern and now auto-removes bucket-owned rank keys before close, but the stronger path is slower than heap and slower than manual cleanup, so container CPU overhead and richer rank APIs remain open. |
...
- The positive compiler/runtime probes store ordinary Scala row objects
  allocated in a child bucket region, rank them by dense key/`Long` priority,
  read the top row, and now also cover the `putWindowRankInBucket` path that
  removes tracked keys automatically before the child bucket closes.
- The negative compiler probe rejects direct heap objects passed to
  `putWindowRank` and `putWindowRankInBucket`.
...
| `StreamWindowIndexedRank` stores child-region objects, ranks them, and auto-removes tracked keys before bucket close | passes compiler probes and native runtime smokes |
...
| Mode | Median elapsed ms | Median GC ms | Median Rift op ms | Region objects | Opens/closes/resets | Peak RSS bytes |
|---|---:|---:|---:|---:|---:|---:|
| heap | 207.038 | 0.000 | 0.000 | 0 | 0 / 0 / 0 | 145768448 |
| rift-checked | 313.572 | 7.689 | 0.307 | 826646 | 41 / 41 / 0 | 94732288 |
...
- It is not a speed win. Checked Rift is slower on elapsed time even though
  Rift runtime operation time is low and RSS is lower. The earlier
  manual-cleanup checked median was `254.050 ms`, so auto cleanup strengthens
  the boundary but adds CPU overhead. Treat this as API/safety evidence plus a
  CPU-overhead warning before integrating the abstraction into DEBS.
