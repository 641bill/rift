...
| Wikimedia enwiki retained clickstream | Clickstream TSV from `clickstream-enwiki-2026-03.tsv.gz`, named clickstream-session workload | 1M `6.50 s`, `784 MB`; 5M `28.77 s`, `1.54 GB`; 10M one-run `24.23 s`, `1.16 GB` | 1M `checked-rift`: `5.81 s`, `138 MB`; 5M `checked-rift`: `27.86 s`, `138 MB`; 10M one-run `checked-rift`: `19.96 s`, `136 MB` | 1M `150.157 ms` / `178.978 ms`; 5M `334.176 ms` / `1066.487 ms`; 10M one-run `4058.998 ms` / `4058.998 ms` | 1M `9.237 ms` / `1.871 ms`; 5M `46.492 ms` / `10.699 ms`; 10M one-run `99.708 ms` / `31.049 ms` | 1M heap fails `512M`; 5M and 10M heap fail `1G`, `768M`, and `512M`; checked rows pass `64M` | 10M checksum `8006060730683441349`, output `9323019` | Real-streaming retained clickstream throughput/RSS/GC/fixed-memory evidence; 10M feasibility strengthens material-GC and fixed-memory evidence, but it is one-run and should be rerun as 3-run before becoming a final median. Local named workload over public Wikimedia data, not an official Wikimedia benchmark artifact. |
...
- Named retained clickstream row now promoted and feasibility-checked to 10M:
...
- Configuration: `records=1000000`, 5M scale-up, and 10M one-run feasibility,
...
| Mode | L1 external s | L1 RSS bytes | L2 median ms | Median GC ms | Max GC ms | Region op ms | Checksum | Output |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| `heap-gc` | `28.77` | `1539489792` | `9459.416` | `334.176` | `1066.487` | `0.000` | `-5539074761685310486` | `4649530` |
| `checked-rift` | `27.86` | `138412032` | `9486.647` | `46.492` | `46.991` | `10.699` | `-5539074761685310486` | `4649530` |
| `checked-region-scoped` | `30.23` | `140886016` | `10156.911` | `733.741` | `740.655` | `0.000` | `-5539074761685310486` | `4649530` |
...
| `checked-rift` | `64M` | pass | `9.37` | `136101888` | cap inherited through `GC_MAXIMUM_HEAP_SIZE` |
| `checked-region-scoped` | `64M` | pass | `10.49` | `136298496` | cap inherited through `GC_MAXIMUM_HEAP_SIZE` |

10M one-run feasibility rows:

| Mode | L1 external s | L1 RSS bytes | L2 median ms | Median GC ms | Max GC ms | Region op ms | Checksum | Output |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| `heap-gc` | `24.23` | `1155743744` | `22877.820` | `4058.998` | `4058.998` | `0.000` | `8006060730683441349` | `9323019` |
| `checked-rift` | `19.96` | `136118272` | `19716.840` | `99.708` | `99.708` | `31.049` | `8006060730683441349` | `9323019` |
| `checked-region-scoped` | `21.43` | `136396800` | `21079.678` | `1703.489` | `1703.489` | `0.000` | `8006060730683441349` | `9323019` |
...
| `checked-rift` | `64M` | pass | `19.39` | `136151040` | cap inherited through `GC_MAXIMUM_HEAP_SIZE` |
| `checked-region-scoped` | `64M` | pass | `21.08` | `136364032` | cap inherited through `GC_MAXIMUM_HEAP_SIZE` |
...
- The 5M scale-up keeps L1 checked Rift slightly faster (`27.86 s` versus
  heap `28.77 s`) and cuts RSS from `1.54 GB` to `138 MB`. Its L2 timed loop
  is essentially tied with heap, but heap max GC grows to `1066.487 ms` and
  heap fails at `1G`, `768M`, and `512M`, while checked Rift completes under a
  `64M` GC heap cap. Use the 5M row primarily as scale-up RSS/fixed-memory and
  GC-tail evidence.
- The 10M feasibility row is stronger on material timed GC: checked Rift is
  L1 `19.96 s`, RSS `136 MB`, versus heap `24.23 s`, RSS `1.16 GB`. L2
  checked Rift is `19716.840 ms` with `99.708 ms` GC and `31.049 ms`
  region-op, versus heap `22877.820 ms` with `4058.998 ms` timed GC. Heap
  fails at `1G`, `768M`, and `512M`; checked rows pass under a `64M` GC heap
  cap. Treat this as a feasibility row until rerun as a 3-run median.
