...
- `unit-tests/native/src/test/scala-next/scala/scala/scalanative/memory/RiftRegionCheckedTest.scala`
  ...
  GraphChi-style subinterval updates through region-owned arrays.
...
- `ENABLE_EXPERIMENTAL_COMPILER=1 sbt "nscplugin3_next/testOnly org.scalanative.RiftRegionCheckedCompilerTest"` passed `35/35` after adding literature-shaped safe API probes for reset record arrays, top-word rooted metadata buffers, GraphChi rooted/unrooted heap metadata, and reset-epoch values stored into outer buffers.
- `ENABLE_EXPERIMENTAL_COMPILER=1 sbt "tests3_next/testOnly scala.scalanative.memory.RiftRegionTest scala.scalanative.memory.RiftRegionCheckedTest"` passed `14/14` after adding top-word and GraphChi runtime smokes.
...
- Checked: region-local object and array allocation through the Scala-next
  ...
  rooted durable heap metadata, but mutable linked-list construction via a
  reassigned local head is still too conservative; use region arrays or
  `ObjectBuffer` in checked code for now.
