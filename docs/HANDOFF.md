...
`69e0be59c97cfcc36173ccb942de1cabd336ae1d`
(`Add checked ObjectBuffer owner methods`)
...
- `ENABLE_EXPERIMENTAL_COMPILER=1 sbt "nscplugin3_next/testOnly org.scalanative.RiftRegionCheckedCompilerTest"` passed `39/39` after adding literature-shaped safe API probes for reset record arrays, top-word rooted metadata buffers, GraphChi rooted/unrooted heap metadata, reset-epoch values stored into outer buffers, mutable local linked-list heads, and owner-token `ObjectBuffer` methods.
- `ENABLE_EXPERIMENTAL_COMPILER=1 sbt "tests3_next/testOnly scala.scalanative.memory.RiftRegionTest scala.scalanative.memory.RiftRegionCheckedTest"` passed `16/16` after adding top-word, GraphChi, mutable linked-list, and owner-token `ObjectBuffer` method runtime smokes.
...
- Still open: direct returned functions are rejected even when they are pure;
  ...
  static immutable referents, and plain receiver-style containers still need a
...
| Phase 8 native GC/region hardening | Started | `HeapRoot` handles give a GC-visible explicit-root path for heap metadata stored from region objects; direct unrooted heap-object constructor arguments, heap aliases, heap field selections, unsafe region-array stores, unsafe owner-token `ObjectBuffer` heap stores, and mutable-head heap retagging are rejected in checked Rift lowering while region-to-region object graphs, simple region-local aliases, stable constructor fields explicitly captured by `{region}`, explicitly captured region arrays, owner-token object buffers, and provenance-preserving mutable local linked-list heads still compile. | Extend or deliberately limit the mixed-reference rule for static immutable heap referents and richer containers; decide whether plain `T^` field reuse and plain receiver-style container operations need a compiler extension or owner-token APIs. |
...
6. The first safe API accept/reject probe slice now passes 30/30 in the
   ...
   fields, static immutable heap referents, and plain receiver-style containers
...
1. Finish the remaining capture/safety guardrails before broadening mixed
   ...
   richer mutable-container provenance beyond local linked-list heads, plain
   receiver-style collection operations beyond owner-token methods, precise
