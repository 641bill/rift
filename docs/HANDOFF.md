...
`1663befb3060232a157019a01a71c97f8a906bc8`
(`Cache Q2 top ten extraction`)
...
required. Region-owned arrays are accepted when reference elements are
explicitly captured, and stores into known region arrays reject unrooted heap
objects. The first checked collection primitive, `RiftRegion.ObjectBuffer`,
...
frontier when a rank update can affect the cached top 10.
...
extraction was committed at `1663befb3`. The
...
- `nscplugin/src/test/scala-next/scala/RiftRegionCheckedCompilerTest.scala`
  ...
  values stored into an outer buffer, and rejecting buffer escape. The latest
  ...
  benchmark objects without the checked mixed-reference guard.
- `unit-tests/native/src/test/scala-next/scala/scala/scalanative/memory/RiftRegionCheckedTest.scala`
  ...
  `HeapRoot` handles, plus an `ObjectBuffer` smoke that stores region objects
  and explicit `HeapRoot` handles through the owner-token API.
...
- `ENABLE_EXPERIMENTAL_COMPILER=1 sbt "nscplugin3_next/testOnly org.scalanative.RiftRegionCheckedCompilerTest"` passed `30/30` after narrowing checked guards to `ScopedRegion`/`StreamingRegion` and adding the trusted-open linked-object regression test.
- `ENABLE_EXPERIMENTAL_COMPILER=1 sbt "tests3_next/testOnly scala.scalanative.memory.RiftRegionTest scala.scalanative.memory.RiftRegionCheckedTest"` passed `12/12` after the checked `ObjectBuffer` update.
...
- Checked: region-local object and array allocation through the Scala-next
  ...
  in checked Rift allocations. Region-owned arrays can be used when reference
  ...
  `RiftRegion.append(region, buffer, value)`. This explicit owner argument is
  intentional in v1 because plain `buffer.append(value)` could not prove the
  same-region relation precisely enough with the current checker.
...
- Still open: direct returned functions are rejected even when they are pure;
  ...
  static immutable referents, and richer/method-style containers still need a
  checked policy or trusted-only labeling. Safe checked code should use `HeapRoot` for
...
6. The first safe API accept/reject probe slice now passes 30/30 in the
   ...
   `{region}` are accepted. Region-owned arrays are accepted when reference
   elements are explicitly captured, and stores into known region arrays reject
   unrooted heap objects. The first checked `ObjectBuffer` container uses an
   explicit owner-token API so ordinary region objects and `HeapRoot` handles
   can be stored while direct heap stores and inner-region-to-outer-buffer
   stores are rejected. Plain `T^` selected fields, static immutable heap
   referents, and ergonomic method-style containers still need a checked policy
   or trusted-only labeling.
...
8. Q2 rank/output attribution is implemented, and the first follow-up fix is
   ...
   rank update cannot affect the cached result. On the 100k single-run
   validation, top-candidate comparisons fell from `4.45M` to `144949`; on the
   1M single-run validation, top 10 recomputed `29983` times out of
   `1000000` calls. The 1M single run measured heap `5367.670 ms`, HPZone
   `5149.720 ms`, and Streaming `5191.038 ms`. These are directional until
   median reruns. A small binary heap for top candidates was tested and
   rejected before commit because it increased comparisons to about `5.39M`.

Next technical milestone:

1. Finish the remaining capture/safety guardrails before broadening mixed
   object graphs: pinned capture diagnostics, static-heap/higher-level
   container mixed-reference policy, plain-field and array-element ergonomics,
...
3. Start DEBS with a narrow measurement-driven plan:
   - Rerun the Q2 top-10 cache checkpoint as 100k and 1M instrumented medians
     before treating the single-run improvement as a headline result.
...
4. For the next DEBS change, rerun 100k and 1M instrumented medians before
   making any headline throughput claim.
...
- DEBS medians for the current Q2 top-10 cache checkpoint.
...
attribution, and the shared Q2 top-10 cache checkpoint. The safest next
technical action is to rerun 100k and 1M instrumented medians for the top-10
cache checkpoint, then either continue Q2 rank/output attribution work or
switch to safe API accept/reject probes for the region object patterns now used
by the literature harnesses.
...
- "The Q2 top-10 cache single runs are final evidence." They are not; they are
  validated correctness/direction checks and need median reruns.
