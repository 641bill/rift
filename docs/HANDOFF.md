...
`60d9fb33a9f908d74b35f05121fdba5d2f7dcc83`
(`Reject direct unrooted heap refs in checked Rift alloc`)
...
`60d9fb33a`. The checked API is not a complete compiler capture-checking
...
- `nscplugin/src/test/scala-next/scala/RiftRegionCheckedCompilerTest.scala`
  ...
  function rejection, the explicit `HeapRoot` region-to-GC metadata path, and
  direct unrooted heap-object constructor-argument rejection.
- `unit-tests/native/src/test/scala-next/scala/scala/scalanative/memory/RiftRegionCheckedTest.scala`
...
- `ENABLE_EXPERIMENTAL_COMPILER=1 sbt "nscplugin3_next/testOnly org.scalanative.RiftRegionCheckedCompilerTest"` passed `13/13` after the direct unrooted heap-reference rejection update.
- `ENABLE_EXPERIMENTAL_COMPILER=1 sbt "tests3_next/testOnly scala.scalanative.memory.RiftRegionTest scala.scalanative.memory.RiftRegionCheckedTest"` passed `10/10` after the direct unrooted heap-reference rejection update.
...
- Still open: direct returned functions are rejected even when they are pure;
  ...
  constructor arguments, not full alias analysis; selected fields, aliases,
...
| Phase 7 capture checking | Started | `RiftRegion.scoped`/`streaming` safe API slice exists. Targeted Scala-next compiler tests now pass for scoped object graphs, for-loop allocation, nested scoped regions, local higher-order consumers, non-escaping closures, return escape rejection, heap retention rejection, nested-region leak rejection, streaming reset escape rejection, conservative returned-function rejection, explicit `HeapRoot` metadata handles, and direct unrooted heap-object constructor-argument rejection. `docs/REPORT_CAPTURE_CHECK.md` records the current checker behavior. | More pinned diagnostics, broader runtime coverage, and alias/field/static-heap mixed-reference policy. |
| Phase 8 native GC/region hardening | Started | `HeapRoot` handles give a GC-visible explicit-root path for heap metadata stored from region objects; direct unrooted heap-object constructor arguments are rejected in checked Rift allocation lowering while region-to-region object graphs still compile. | Extend or deliberately limit the mixed-reference rule for aliases, field selections, static immutable heap referents, and containers; broaden mixed-reference tests. |
...
6. The first safe API accept/reject probe slice now passes 13/13 in the
   ...
   Aliases, field selections, static immutable heap referents, and containers
...
1. Finish the remaining capture/safety guardrails before broadening mixed
   object graphs: pinned capture diagnostics, alias/field/static-heap
   mixed-reference policy, precise returned-closure support beyond the v1
   rejection, and explicit HPZone/trusted labels for unsafe cases.
