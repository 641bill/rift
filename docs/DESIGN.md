...
- DEBS performance is provisional and not yet a final application-level Rift
  ...
  is again a small share of total elapsed time. The latest 1M 3-run median has
  heap at `9746.536 ms`, HPZone at `9441.064 ms`, and Streaming at
  `9442.026 ms`; Streaming GC is `306.311 ms` versus heap `312.151 ms`, with
  much lower Rift RSS on the bounded sample. SafeZone/Commix/full-scale
...
- The current Q1/Q2 direction follows that boundary: timestamp-bucket window
  ...
  region-backed in Rift modes. Latency arrays remain heap metadata. Primitive
  packed keys are used in the shared logic to avoid accidental heap objects in
  the hot path.
