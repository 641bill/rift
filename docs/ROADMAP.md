...
1. Preserve the checked RunBoth medians as the latest bounded Phase 5/7
   ...
   controls and repeated full-month medians after the Q1 rank lifetime fix.
2. Treat the Q1 rank lifetime fix as a memory-diagnostic result, not a final
   headline. The next narrow evidence step is to rerun full-month heap/checked
   controls without family instrumentation and add SafeZone full-month controls.
   Inspect the remaining checked parent payload only if those controls still
   show a memory problem.
