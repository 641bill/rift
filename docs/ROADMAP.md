...
- Q1 checked rank-refresh churn now has a bounded window-rank arena fix:
  ...
  but it is still not a speed win. The next step is continuing container CPU
  reduction or adding a richer comparator/hash-key rank API; bounded Q2
...
4. DEBS safe-API probes now cover Q1 output/ranking materialization, Q1
   ...
   manual-cleanup path. The new entry-cleanup callback removes duplicate
   checked-side bucket key lists and improves the focused default median, but
   not enough for direct DEBS integration. The Q2 CPU substep diagnostic did not
   reproduce bounded same-operation Q2 overhead, so the next DEBS safe-API
   target should be continued checked-container CPU work or a richer rank API,
   not direct wholesale Q1 integration.
...
- Rift already has final DEBS application proof. The bounded-sample top-cache
  ...
  entry-cleanup callback reduces that overhead but does not remove it. Richer
  comparator/hash-key rank collections, optional full-month SafeZone controls,
  and stronger safe API controls are still missing. Bounded Q2 same-operation
  overhead is not currently reproduced by the opt-in substep diagnostic.
