...
the latest checked-rank API slices included. `RegionLongIndexedPriorityQueue`
provides standalone hash-keyed checked rank state for arbitrary `Long` keys,
with the same owner-token heap-store guard as the dense indexed queue.
`StreamWindowLongIndexedRank` now composes that queue with stream-bucket close
