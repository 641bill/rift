...
- A closeable SafeZone mode now exists for Q1/Q2 data-structure placement. The
  first 100k/1M controls match heap output for `SAFEZONE_ROOTS_MODE=0` and
  `SAFEZONE_ROOTS_MODE=1`, but they are single-run evidence. Bounded 3-run
  SafeZone medians and then full-month SafeZone controls remain open.
...
- Rerun 100k and 1M instrumented matrices with medians after each change that
  ...
  1M medians.
- Add bounded medians for the new SafeZone mode, including improved roots mode.
