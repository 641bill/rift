...
`91/91`, and the native checked runtime suite is `36/36`.
...
reusable checked operator but is slower than the fair specialized heap control
at 1M, while reducing GC and RSS. Q5 diagnostics show top scanning is a shared
cost rather than the checked-specific loss. This supports building reusable
region-backed stream/join/window operators, but every specialized API needs a
fair heap counterpart before it becomes performance evidence.
