...
- Q1 checked rank-refresh churn now has a bounded window-rank arena fix:
  ...
  slower than heap. The next step is checked Q2 overhead investigation with
  identical operation counts or a lower-overhead/richer rank API.
...
4. DEBS safe-API probes now cover Q1 output/ranking materialization, Q1
   ...
   slower than heap in its first median. The next DEBS safe-API target should
   therefore be either checked Q2 same-operation overhead diagnosis or a
   lower-overhead/richer rank API, not direct wholesale Q1 integration.
...
- Rift already has final DEBS application proof. The bounded-sample top-cache
  ...
  comparator/hash-key rank collections, checked Q2 overhead explanation,
  optional full-month SafeZone controls, and stronger safe API controls are
  still missing.
