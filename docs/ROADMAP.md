...
- Q1 checked rank-refresh churn now has a bounded window-rank arena fix:
  ...
  next step is a higher-level checked rank/window collection and checked Q2
  overhead investigation with identical operation counts.
...
4. DEBS safe-API probes now cover Q1 output/ranking materialization, Q1
   ...
   a reusable `ChildBucket` wrapper, and the reusable `StreamBucketArena`
   primitive. The next DEBS safe-API target should be a higher-level checked
   rank/window collection or stronger typed cleanup/unlink obligation, not
   another one-off benchmark-only pattern.
...
- Rift already has final DEBS application proof. The bounded-sample top-cache
  ...
  primitive is now implemented, but higher-level rank/window collections,
  checked Q2 overhead explanation, optional full-month SafeZone controls, and
