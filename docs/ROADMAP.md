...
- Q1 checked rank-refresh churn now has a bounded window-rank arena fix:
  ...
  The next step is reducing ownership-bookkeeping overhead or adding a richer
  comparator/hash-key rank API; bounded Q2 same-operation overhead should not
  be optimized further until a clean or full-month diagnostic identifies a
  specific substep.
...
4. DEBS safe-API probes now cover Q1 output/ranking materialization, Q1
   ...
   manual-cleanup path. The Q2 CPU substep diagnostic did not reproduce bounded
   same-operation Q2 overhead, so the next DEBS safe-API target should be
   reducing ownership-bookkeeping overhead or adding a richer rank API, not
   direct wholesale Q1 integration.
...
- Rift already has final DEBS application proof. The bounded-sample top-cache
  ...
  and auto cleanup currently adds CPU overhead versus manual cleanup. Richer
