...
- Rerun 100k and 1M instrumented matrices with medians after each change that
  ...
  1M medians.
- Reduce Q1 checked rank-refresh object churn without one child region per
  route, and investigate checked Q2 overhead with identical operation counts.
...
- Rift already has final DEBS application proof. The bounded-sample top-cache
  ...
  and SafeZone 1M controls plus checked process diagnostics now exist. Q1
  rank-refresh overhead reduction, checked Q2 overhead explanation, optional
