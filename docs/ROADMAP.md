...
| Phase 6: literature-aligned methodology evidence | Started | `DATAFLOW_REGION_MATRIX.md`, `STREAMFLEX_REGION_MATRIX.md`, `YAK_REGION_MATRIX.md`, `STANCU_REGION_MATRIX.md`, `NEXMARK_REGION_MATRIX.md`, `COMMON_CRAWL_WET_MATRIX.md`, `WIKIMEDIA_REGION_MATRIX.md`, `LINEAR_ROAD_REGION_MATRIX.md`, `YAHOO_AD_REGION_MATRIX.md`, `RIOTBENCH_REGION_MATRIX.md`, `STREAM_GC_BENCHMARK_CANDIDATES.md`, and `STREAM_BENCHMARK_LADDER.md` now cover Broom-style dataflow, StreamFlex-style latency/throughput, Yak-style control/data epochs including grouped sort, top-word/filter, and GraphChi-like subinterval updates plus runtime and memory-API-level promotion/escape proxies, Stancu-style transaction accounting, NEXMark-lite and Beam-default generated-profile streaming including Q3/Q4/Q9/Q11, generated and real/preloaded Common Crawl WET, Wikimedia clickstream, generated and official Linear Road position/toll probes, Yahoo-style ad-stream probes, RIoTBench-style IoT ETL/statistics probes, and a ranked stream-GC benchmark ladder. Dataflow now includes checked SELECT/AGGREGATE/JOIN modes. NEXMark-lite Q1/Q2 show modest 1M elapsed wins with lower GC, generic Q8 is the strongest checked NEXMark-lite row so far, Q0 is a near tie, and Q5 is not yet a window-aggregate win. Beam-default Q0/Q1/Q3/Q8/Q11 are the useful new profile rows; Q3 is the best checked expanded-query row. Real enwiki clickstream, real Common Crawl WET, and official Linear Road preloaded rows are ceiling results. Yahoo Q2 cuts GC but does not beat heap elapsed at 1M. RIoTBench-style q1 stresses heap GC and Rift beats heap, but improved SafeZone remains faster. | Keep these labeled as methodology reproductions or real-input probes, not exact paper artifacts. The checked Dataflow result is strong local Broom-style safe-API evidence, but still not exact Broom/Naiad. The Yak top-word/filter result is a strong local Rift-vs-heap and modest Rift-vs-improved-SafeZone result; GraphChi-like subintervals show Rift-vs-heap but not Rift-vs-improved-SafeZone; grouped sort is a modest same-program allocation-placement win; runtime promotion is negative on elapsed time and motivates static checked boundaries. The Q8 reusable join API is framework progress; packed counts narrowed the fair 1M gap but it still failed the speed gate against `heap-join-api` (`20.987 ms` vs `17.393 ms`). The additive `StreamWindowFold` gate also failed (`118.726 ms` checked vs `103.244 ms` heap at 1M). Do not force Common Crawl, Wikimedia, Linear Road, Yahoo, or RIoTBench into headline claims; next work should reduce focused checked-operator overhead and use NEXMark Q3/Q8 plus Yahoo/RIoTBench controls as regression/profile checks. |
...
After the real-input ladder, the safest technical next action is:
...
2. Keep NEXMark Beam-default Q3/Q8 as the current positive checked
   application-profile checks, with Q0/Q1/Q11 as trusted-runtime profile rows
   and Yahoo Q2 / RIoTBench q1 as GC-pressure regression controls. These are
   generated-profile rows, not exact Beam/Yahoo/RIoTBench runner evidence, but
   they preserve the same logical heap/Rift program and expose the relevant
   stream-lifetime shapes.
