...
1. Treat real enwiki clickstream, real Common Crawl WET, and official Linear
   ...
   measured timed GC is `0.000 ms` in those timed sections, so they should not
   drive application-specific tuning.
2. Keep NEXMark Beam-default Q1/Q8 as the current positive application-profile
   checks. They are generated-profile rows, not exact Beam runner evidence, but
   they preserve the same logical heap/Rift program and expose structured
   lifetime wins.
