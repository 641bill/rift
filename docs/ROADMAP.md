...
`96/96`, and the native checked runtime suite is `38/38`.
...
2. Treat the Q1 rank lifetime fix as a memory result, not a final headline.
   The process diagnostic explains the next narrow target: Q2 counts are
   identical between heap and checked, while Q1 checked rank creations rise to
   one per rank refresh. Do not use one child region per active Q1 route: a
   100k probe reduced rank-object churn but opened `69220` regions and raised
   RSS to `113721344` bytes.
...
4. DEBS safe-API probes now cover Q1 output/ranking materialization, Q1
   ...
   a reusable `ChildBucket` wrapper, the reusable `StreamBucketArena`
   primitive, and `StreamWindowIndexedRank`. The focused
   `CheckedStreamWindowRankMatrix` now shows that the abstraction works and can
   auto-remove bucket-owned rank keys before close, but the stronger
   close-discipline path is slower than heap and slower than the earlier
   manual-cleanup path. The entry-cleanup callback removes duplicate
   checked-side bucket key lists and improved the focused default median, but
   not enough for direct DEBS integration. The remove-with-value close primitive
   is correctness/usefulness cleanup, not a measured speed fix. The Q2 CPU
   substep diagnostic did not reproduce bounded same-operation Q2 overhead, so
   the next DEBS safe-API target should be continued checked-container CPU work
   or a richer rank API, not direct wholesale Q1 integration.
