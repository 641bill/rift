...
- Q1 checked rank-refresh churn now has a bounded window-rank arena fix:
  ...
  layer, and the focused matrix shows it works with matching checksums but is
  slower than heap. The next step is a lower-overhead/richer rank API and
  stronger close discipline; bounded Q2 same-operation overhead should not be
  optimized further until a clean or full-month diagnostic identifies a
  specific substep.
...
4. DEBS safe-API probes now cover Q1 output/ranking materialization, Q1
   ...
   `CheckedStreamWindowRankMatrix` now shows that the abstraction works but is
   slower than heap in its first median. The Q2 CPU substep diagnostic did not
   reproduce bounded same-operation Q2 overhead, so the next DEBS safe-API
   target should be a lower-overhead/richer rank API plus stronger bucket/window
   close discipline, not direct wholesale Q1 integration.
...
- Rift already has final DEBS application proof. The bounded-sample top-cache
  ...
  primitive and dense-key `StreamWindowIndexedRank` are now implemented, but
  the focused stream-window rank matrix is slower than heap. Richer
