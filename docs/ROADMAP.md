...
- Add negative compile tests for return escape, closure capture escape,
  ...
  `DataflowRegionMatrix` now also has a SELECT-only checked safe-API mode; the
  aggregate and join checked ports remain open because they need a clean table
  provenance story.
