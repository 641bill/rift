...
- The active-memory diagnostic shows the remaining full-month RSS problem is
  ...
  are near the `128 MiB` pool cap. The next DEBS step should attribute that
  live region payload by operator/region family and shorten or compact the
  largest live lifetimes without changing the logical query.
...
1. Preserve the checked RunBoth medians as the latest bounded Phase 5/7
   ...
   descheduled. The pool-cap and `ChildBucket` follow-ups now give a same-order
   full-month 3-run heap/checked median, but still need SafeZone full-month
   controls and per-region-family memory attribution.
2. Use the active-memory diagnostic to choose the next narrow implementation
   target. It shows the full-month RSS regression is mostly live checked-region
   payload at current lifetimes, not closed-slab pool retention. Attribute live
   bytes by Q1/Q2 region family before changing layouts.
...
- Rift already has final DEBS application proof. The bounded-sample top-cache
  ...
  median, but SafeZone full-month controls, per-region-family memory
