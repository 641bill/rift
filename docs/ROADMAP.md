...
| Phase 6: literature-aligned methodology evidence | Started | `DATAFLOW_REGION_MATRIX.md`, `STREAMFLEX_REGION_MATRIX.md`, `YAK_REGION_MATRIX.md`, `STANCU_REGION_MATRIX.md`, and `NEXMARK_REGION_MATRIX.md` now cover Broom-style dataflow, StreamFlex-style latency/throughput, Yak-style control/data epochs including grouped sort, top-word/filter, and GraphChi-like subinterval updates plus runtime and memory-API-level promotion/escape proxies, Stancu-style transaction accounting, and a first NEXMark-lite stream-processing ladder. Dataflow now includes checked SELECT/AGGREGATE/JOIN modes. NEXMark-lite Q1/Q2 show modest 1M elapsed wins with lower GC, Q8 is the strongest checked NEXMark-lite row so far, Q0 is a near tie, and Q5 is not yet a window-aggregate win. | Keep these labeled as methodology reproductions, not exact paper artifacts. The checked Dataflow result is strong local Broom-style safe-API evidence, but still not exact Broom/Naiad. The Yak top-word/filter result is a strong local Rift-vs-heap and modest Rift-vs-improved-SafeZone result; GraphChi-like subintervals show Rift-vs-heap but not Rift-vs-improved-SafeZone; grouped sort is a modest same-program allocation-placement win; runtime promotion is negative on elapsed time and motivates static checked boundaries. Next broader-stream work should either factor the Q8 join-window shape into a reusable operator API or profile Q5 before moving to Wikimedia/Linear Road/Common Crawl. |
...
DEBS: Q1 conversion and checked Q2 selection are modest 1M wins, Q8 window
join is a stronger checked win, Q0 passthrough is a near tie, and Q5 hot-items
loses slightly. This supports building reusable region-backed stream/join
operators before more DEBS-specific tuning.
