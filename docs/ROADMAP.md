...
| Honest controls | GH Archive q2 is a tie and NEXMark q9 favors the scoped backend over checked stream. Checked window-fold was a regression/control row in the published docs, but the current focused rerun now has checked fold narrowly ahead of heap (`97.321 ms` versus `99.302 ms`) and the first NEXMark q5 fold API gate also passes (`419.855 ms` versus heap `470.288 ms`). |

## Current Validation Correction: 2026-06-01

The child implementation base head (`685f2b67e`) plus the local validation fix
is a validated capture-directed inference milestone, not a validated "full
region inference" milestone. The fix gates broad automatic local-escape scope
insertion behind `-P:scalanative:riftInferAutomaticScopes` and keeps the GenNIR
automatic-region hook default-off. Validation now passes: compiler `718/718`,
runtime `322/322`, and `sandbox3_next` compile passed.

The current validated default includes broad explicit-owner and owner-token
placement for ordinary `new`, arrays, options, tuples, `Either`, generic
wrappers, closure objects, and narrow captured-owner closure-body effects. It
does not include promoted automatic region creation/deallocation for arbitrary
ordinary Scala code.

Remaining roadmap:

1. Rerun NEXMark q5 fold API under Beam defaults if it is going into a
   presentation-facing table; otherwise move to the next fold-shaped
   application only when it needs generic fold state.
2. Rebuild the opt-in automatic local-escape wrapping path with allocation-site
   precision, library exclusion, exception-safe close, and runtime
   allocation-stat proof before promoting automatic scopes.
3. Finish broad closure/effect summaries: escaping-closure summaries,
...
4. Add primitive box and boxed-key placement only after an allocation-zone design
   preserves identity/cache semantics.
5. Expand library-created allocation support for iterators, collection nodes,
   strings, buffers, parser helpers, wrappers, erased generic paths, and common
   Scala collection nodes.
6. Keep runtime checks until compiler/runtime probes prove active-handle,
   stale-token, owner, and close-order invariants.
7. Rerun the latest selected matrix from a clean committed tree before using the
   current dirty-run numbers as final publication claims.
8. Mechanize the core proof for containment, heap roots, owner tokens, and
   close/reset safety.
9. Treat JVM, Scala.js, Wasm, and analysis-only backends as future tracks; only
...
follow-up first removed measured GC and lowered RSS but failed the focused 1M
speed gate; the 2026-06-01 rerun after redundant open-check removal now passes
that focused gate narrowly (`97.321 ms` checked versus `99.302 ms` heap). This supports
