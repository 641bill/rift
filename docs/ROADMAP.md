...
- Q1 checked rank-refresh churn now has a bounded window-rank arena fix:
  ...
  next step is to generalize this into a reusable checked rank/window API and
  investigate checked Q2 overhead with identical operation counts.
...
- Rift already has final DEBS application proof. The bounded-sample top-cache
  ...
  without one child region per route. SafeZone 1M controls plus checked process
  diagnostics now exist, but reusable rank/window APIs, checked Q2 overhead
