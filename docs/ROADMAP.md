...
| Phase 6: literature-aligned methodology evidence | Started | `DATAFLOW_REGION_MATRIX.md`, `STREAMFLEX_REGION_MATRIX.md`, `YAK_REGION_MATRIX.md`, and `STANCU_REGION_MATRIX.md` now cover Broom-style dataflow, StreamFlex-style latency/throughput, Yak-style control/data epochs with runtime and memory-API-level promotion/escape proxies, and Stancu-style transaction accounting. | Keep these labeled as methodology reproductions, not exact paper artifacts. The Yak runtime-promotion result is negative on elapsed time, so use it as motivation for static checked boundaries, not as a Rift speedup claim. Build fair Rift-backed collection/operator API before claiming a Broom/parallel-collections API comparison. |
...
- `evidence/YAK_REGION_MATRIX.md` records Yak-style word-count, graph-step, and
  promotion/escape epoch/control-data split workloads. It now includes a
  `yak-runtime` proxy for a pure runtime-managed epoch discipline on Scala
  Native and a `RiftRegion.RuntimeEpoch` memory API for rare escaping data
  objects. The promotion proxy removes measured heap GC but is slower than heap
  on elapsed time at the current pressure size.
...
3. Decide whether the next DEBS target is rank maintenance/output attribution
...
4. If continuing DEBS, keep the fair-backend rule: same logical query
   algorithm, heap backend with `new`, Rift backend with `region.alloc` and
   region close/reset at the same lifetime boundary.
5. Rerun 100k and 1M instrumented medians after every further DEBS change.
