...
`ChildWindow` heap control object per bucket. The full-month evidence is still
single-run, and the reverse-order heap row was invalid due descheduling, but
the checked row stayed fast when run first (`68.435 s`). RSS did not improve;
checked full-month RSS rose to `866-911 MiB` in these runs, so the result is
an elapsed/checker-overhead improvement only.
...
- Next, either rerun full-month heap/checked medians with the pool cap in place
  or attack the remaining full-month memory pressure in long-lived
  parent-stream state. Any new DEBS change must keep heap and Rift paths as the
  same logical program and make the allocation/lifetime boundary explicit.
...
1. Preserve the checked RunBoth medians as the latest bounded Phase 5/7
   ...
   descheduled. The pool-cap follow-up gives a credible same-run full-month
   heap/checked control, but still needs repeated full-month medians.
...
- Rift already has final DEBS application proof. The bounded-sample top-cache
  ...
  pool-cap same-run full-month control gives a usable single full-scale data
  point, but SafeZone, repeated full-month medians, and stronger safe API
