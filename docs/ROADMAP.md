...
`streamBucketRegion(...)`, and stale-current `isOpen` checks while keeping
public low-level APIs defensive. New stale-bucket probes and compiler guards
validate the boundary: compiler tests are now `100/100`, runtime tests are
