...
- Q1 checked rank-refresh churn now has a bounded window-rank arena fix:
  ...
  layer. The next step is a focused benchmark or integration attempt plus
  checked Q2 overhead investigation with identical operation counts.
...
4. DEBS safe-API probes now cover Q1 output/ranking materialization, Q1
   ...
   primitive, and `StreamWindowIndexedRank`. The next DEBS safe-API target
   should be a focused stream-window rank benchmark/integration or stronger
   typed cleanup/unlink obligation, not another one-off benchmark-only pattern.
...
- Rift already has final DEBS application proof. The bounded-sample top-cache
  ...
  richer comparator/hash-key rank collections, checked Q2 overhead
  explanation, optional full-month SafeZone controls, and stronger safe API
