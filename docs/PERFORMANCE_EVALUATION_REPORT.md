...
| 4 | More real log/NDJSON streams | LogHub BGL/HDFS/Spark and GH Archive are modest wins but not GC-heavy. | Try Windows/Thunderbird logs, GDELT, or security logs only if they materialize more per-record objects than the current line/template paths. |
