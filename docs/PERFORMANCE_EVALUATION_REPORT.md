...
`evidence/THEODOLITE_POWER_REGION_MATRIX.md` now includes the first stronger
real-streaming retained time-series row, `q3-retained-uc4`, over the compressed
UCI household-power archive member. It allocates and retains one measurement
plus twelve UC4-style hierarchy contribution objects per usable record, then
bulk-closes the epoch/window. On the full local stream (`2049280` usable
records), natural heap is L1 `16.85 s`, RSS `207 MB`; checked Rift stream epoch
is L1 `14.06 s`, RSS `26.6 MB`; checked scoped is `14.88 s`, RSS `26.7 MB`.
L2 heap is `4504.438 ms` with `335.309 ms` median GC and `482.380 ms` max GC;
checked Rift is `3769.962 ms` with `31.570 ms` median GC and `26640640`
region objects. Heap completes at `128M` but slows to `17.90 s`, fails at
`64M`, while checked Rift completes under `64M` at `14.27 s`. This is still a
local Theodolite-style kernel paired with real input, not exact Theodolite
artifact reproduction, but it is now the strongest current real-streaming
retained-object GC/RSS/fixed-memory row.
`evidence/LOGHUB_RETAINED_SESSION_MATRIX.md` now also includes a named
Wikimedia retained clickstream-session workload over the compressed public
enwiki clickstream TSV. It streams rows from
`clickstream-enwiki-2026-03.tsv.gz`, derives session keys from source article,
target article, and link kind, retains event plus aggregate entries until
epoch close, and keeps durable counters on the heap/control path. At 1M
streamed rows, natural heap is L1 `6.50 s`, RSS `784 MB`; checked Rift is
`5.81 s`, RSS `138 MB`; checked scoped is `6.47 s`, RSS `138 MB`. L2 heap is
`2038.262 ms` with `150.157 ms` median timed GC, while checked Rift is
`1952.438 ms` with `9.237 ms` timed GC and `1.871 ms` region-op time. Heap
fails even at a `512M` cap, while checked rows complete under `128M` and
`64M`. The 10M 3-run scale-up keeps checked Rift L1 faster (`59.37 s` versus
heap `62.52 s`) and cuts RSS from `1.80 GB` to `138 MB`; L2 checked Rift is
`19635.995 ms`, GC `96.069 ms`, and region-op `24.329 ms`, versus heap
`20103.863 ms` with `851.023 ms` timed GC. Heap fails at `1G`, `768M`, and
`512M`, while checked rows complete under a `64M` GC heap cap. This is a local
named retained clickstream workload over public Wikimedia data, not an
official Wikimedia benchmark artifact, but it is now a positive
real-streaming throughput/RSS/GC/fixed-memory scale-up row.
A one-run full local file feasibility pass over all `35862259` compressed rows
also completes: checked Rift is L1 `69.87 s`, RSS `136 MB`, versus heap
`74.23 s`, RSS `2.30 GB`; L2 checked Rift is `71149.579 ms`, GC
`318.165 ms`, and region-op `79.786 ms`, versus heap `72750.014 ms` with
`7122.811 ms` timed GC. Keep the 10M x3 row as the report-grade median until
the full-file row is repeated.
...
The retained UC4 q3 follow-up strengthens this from modest q2 evidence to a
serious retained-object stream row: full local q3 checked Rift stream is
`14.06 s`, RSS `26.6 MB`, versus heap `16.85 s`, RSS `207 MB`; L2 removes about
`304 ms` median timed GC and heap fails under a `64M` cap while checked Rift
completes.
...
| Real-input search direction | The search is tracked in `evidence/REAL_INPUT_BENCHMARK_SEARCH.md`. Yak `graphreal` over SNAP LiveJournal is now the top prior-work-shaped real-input ladder row, and AskUbuntu `topwordreal` is the first real text/top-word follow-up. The new AskUbuntu `streaming-file` row confirms bounded file replay but also confirms that XML/text scanning can dominate when heap GC is small. Theodolite retained UC4 q3 and Wikimedia retained clickstream-session are now the best true streaming retained-object rows; LogHub HDFS top-template streaming remains the best true streaming retained/top-k row; richer HDFS q3 streaming confirms an RSS win but not an elapsed win. Wikimedia now has one-run full-file feasibility over all `35862259` compressed rows, but the 10M x3 row remains the report-grade median until the full-file row is repeated. Next adjacent targets are larger Stack Exchange/StackOverflow text if disk/time allow, SNAP Twitter-2010 only if disk/time allow, richer LogHub/session joins, and public text/security NDJSON streams that naturally materialize retained objects. DSPBench Spike, Fraud, and Log Processing q0/q1/q2 are implemented; LogHub BGL, HDFS, Spark, and Windows q1/q2/q3/top-template variants have been tried; RIoTBench/MHEALTH q1/q2 has been tried; Theodolite UC2/UC4-style q1/q2/q3 has been tried over the real UCI power trace, with q3 promoted and q1/q2 retained as modest/control evidence. |
...
> q1/q2 stressor. The real-streaming retained-object ladder now has positive
> Theodolite and Wikimedia rows; the remaining research problem is broadening
> that result across more public streams and checked operators.
...
| Negative/ceiling | Heap is fastest and there is no material GC/RSS/tail benefit at the measured scale. | Pipeline surrogate, old preloaded Wikimedia counter row, Linear Road. |
...
The project now has real-streaming retained-object case-study candidates:
Theodolite retained UC4 and Wikimedia retained clickstream-session. It still
does not have an exact prior-system artifact reproduction, and the broader
real-input suite remains mixed because many public streams are parser/hash
dominated rather than GC dominated.

GH Archive remains the clearest parser/RSS control. The legacy file-backed
...
| Wikimedia real clickstream counter | old real preloaded TSV counter row | heap `126.800 ms` | improved `149.062 ms` | Streaming `157.449 ms` | Heap wins; ceiling/control for this old preloaded counter shape. Do not confuse with the named retained clickstream-session streaming row. |
| Wikimedia retained clickstream-session | real streaming compressed TSV | 1M L1 heap `6.50 s`, RSS `784 MB`; 10M x3 L1 heap `62.52 s`, RSS `1.80 GB`; full one-run heap `74.23 s`, RSS `2.30 GB`; 10M L2 heap GC `851.023 ms`, max GC `885.434 ms`; full one-run heap GC `7122.811 ms` | 10M checked scoped `62.33 s`, RSS `139 MB`; full one-run checked scoped `76.83 s`, RSS `137 MB` | 1M checked Rift `5.81 s`, RSS `138 MB`; 10M x3 checked Rift `59.37 s`, RSS `138 MB`; full one-run checked Rift `69.87 s`, RSS `136 MB`; 10M L2 GC `96.069 ms`, region op `24.329 ms`; full one-run L2 GC `318.165 ms`, region op `79.786 ms` | Named retained clickstream workload over public Wikimedia data. 1M is throughput/RSS/GC/fixed-memory positive; 10M is report-grade throughput/RSS/GC/fixed-memory scale-up evidence. Full-file one-run confirms feasibility over all `35862259` compressed rows but is provisional until repeated. Heap fails at `1G`/`768M`/`512M`, checked rows pass under `64M`. Not an official Wikimedia benchmark artifact. |
...
| Real WET/WAT/old preloaded Wikimedia/Linear Road rows mostly heap-fastest or median-GC-zero; LogHub/GH Archive are only modest wins. | These older input shapes, as wired, do not stress GC enough for the flagship claim. The named retained Wikimedia clickstream-session row is a newer positive real-streaming exception. GH Archive q1 and LogHub BGL q2 are memory-budget/tail/modest-throughput candidates rather than decisive uncapped throughput wins. |
...
| Control | Old preloaded Wikimedia counter row, Linear Road, Yahoo, DEBS | Useful regression/ceiling rows. | Revisit only after a checked operator/backend change alters allocation behavior. |
...
| "Checked Rift already has a final application win." | Partly supported: named retained rows such as Theodolite UC4 and Wikimedia clickstream-session now win, but the broader public-stream suite and exact prior-system artifact story remain open. |
...
| "Real stream data is always GC-heavy." | False. Current real WET, old preloaded Wikimedia counter, and Linear Road rows are mostly ceiling controls; retained-state rows such as Theodolite UC4 and Wikimedia clickstream-session are the positive cases. |
