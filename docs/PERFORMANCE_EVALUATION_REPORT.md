...
Yak/Hadoop top-word family. At 10M real tokens x5, direct checked epoch scoped
is `3.86 s` and `94 MB` RSS versus heap `4.19 s` and `428 MB`; the matching L2
row is `207.490 ms` with no timed GC versus heap `269.491 ms` with
`23.715 ms` median timed GC. The reusable top-k scoped row is still only a
slight L1 elapsed win (`4.12 s`) and trails direct epoch, so it remains an API
cost target rather than the fastest text-top-word implementation.
...
| Real-input search direction | The search is tracked in `evidence/REAL_INPUT_BENCHMARK_SEARCH.md`. Yak `graphreal` over SNAP LiveJournal is now the top prior-work-shaped real-input ladder row, and AskUbuntu `topwordreal` is the first real text/top-word follow-up. Next adjacent targets are larger Stack Exchange/StackOverflow text if disk/time allow, richer LogHub top-template/session mining, SNAP Twitter-2010 only if disk/time allow, and public text/security NDJSON streams that naturally materialize retained objects. DSPBench Spike, Fraud, and Log Processing q0/q1/q2 are implemented; LogHub BGL, HDFS, Spark, and Windows q1/q2/q3/top-template variants have been tried; RIoTBench/MHEALTH q1/q2 has been tried. Theodolite source is cloned, but its official workload generator is simulated input, so pair UC2/UC4 logic with a real industrial-energy trace before claiming real-input evidence. |
