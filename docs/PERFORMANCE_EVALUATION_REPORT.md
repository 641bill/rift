...
inside each run instead of preloading parsed primitive arrays. At 1M q2
hierarchical records, checked scoped epoch is an L1 throughput/RSS/fixed-memory
win (`10.07 s`, `29.6 MB` RSS) versus heap (`10.92 s`, `147 MB` RSS). The L2
row is `2436.713 ms` with `54.154 ms` median GC for checked scoped versus heap
`2479.703 ms` with `143.088 ms` median GC. Parser/string allocation still
creates heap GC in all modes, so this is useful real-streaming evidence but
not a huge-GC flagship.
