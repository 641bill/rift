...
| Wikimedia retained clickstream-session | real streaming compressed TSV | 1M L1 heap `6.50 s`, RSS `784 MB`; 10M x3 L1 heap `62.52 s`, RSS `1.80 GB`; 10M L2 heap GC `851.023 ms`, max GC `885.434 ms` | 10M checked scoped `62.33 s`, RSS `139 MB` | 1M checked Rift `5.81 s`, RSS `138 MB`; 10M x3 checked Rift `59.37 s`, RSS `138 MB`; 10M L2 GC `96.069 ms`, region op `24.329 ms` | Named retained clickstream workload over public Wikimedia data. 1M is throughput/RSS/GC/fixed-memory positive; 10M is report-grade throughput/RSS/GC/fixed-memory scale-up evidence. Heap fails at `1G`/`768M`/`512M`, checked rows pass under `64M`. Not an official Wikimedia benchmark artifact. |
