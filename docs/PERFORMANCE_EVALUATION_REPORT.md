...
`64M`. The 5M scale-up keeps checked Rift L1 slightly faster (`27.86 s`
versus heap `28.77 s`) and cuts RSS from `1.54 GB` to `138 MB`; L2 loop
throughput is essentially tied, but heap max GC grows to `1066.487 ms` and
heap fails at `1G`, `768M`, and `512M`, while checked rows complete under a
`64M` GC heap cap. This is a local named retained clickstream workload over
public Wikimedia data, not an official Wikimedia benchmark artifact, but it is
now a positive real-streaming throughput/RSS/GC/fixed-memory row at 1M and
scale-up RSS/fixed-memory/GC-tail row at 5M.
...
| Wikimedia retained clickstream-session | real streaming compressed TSV | 1M L1 heap `6.50 s`, RSS `784 MB`; 5M L1 heap `28.77 s`, RSS `1.54 GB`; 5M L2 heap GC `334.176 ms`, max GC `1066.487 ms` | 5M checked scoped `30.23 s`, RSS `141 MB` | 1M checked Rift `5.81 s`, RSS `138 MB`; 5M checked Rift `27.86 s`, RSS `138 MB`; 5M L2 GC `46.492 ms`, region op `10.699 ms` | Named retained clickstream workload over public Wikimedia data. 1M is throughput/RSS/GC/fixed-memory positive; 5M is RSS/fixed-memory/GC-tail scale-up evidence with essentially tied L2 loop throughput. Heap fails at `1G`/`768M`/`512M`, checked rows pass under `64M`. Not an official Wikimedia benchmark artifact. |
