...
`64M`. This is a local named retained clickstream workload over public
Wikimedia data, not an official Wikimedia benchmark artifact, but it is now a
positive real-streaming throughput/RSS/GC/fixed-memory row.
...
| Wikimedia retained clickstream-session | real streaming compressed TSV | L1 heap `6.50 s`, RSS `784 MB`; L2 heap `2038.262 ms`, GC `150.157 ms` | checked scoped `6.47 s`, RSS `138 MB` | checked Rift `5.81 s`, RSS `138 MB`; L2 GC `9.237 ms`, region op `1.871 ms` | Named retained clickstream workload over public Wikimedia data. Throughput/RSS/GC/fixed-memory win; heap fails at `512M`, checked rows pass under `128M`/`64M`. Not an official Wikimedia benchmark artifact. |
