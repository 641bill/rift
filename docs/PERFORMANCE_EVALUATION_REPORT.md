...
(`3.77 s`, `24.9 MB` RSS) versus heap (`3.87 s`, `75.3 MB` RSS). The L2 row is
`967.144 ms` with `0 ms` median GC for checked scoped versus heap
`1007.030 ms` with `19.932 ms` median GC. This is useful real-streaming
evidence and also a methodology lesson: parser allocation can inflate apparent
GC pressure if not controlled.
