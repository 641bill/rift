...
| Real-streaming retained-object case studies | Theodolite retained UC4 full local stream: checked Rift stream `14.06 s`, `26.6 MB` RSS vs heap `16.85 s`, `207 MB`, L2 heap GC `335.309 ms`, heap fails at `64M`. Wikimedia retained clickstream-session: 1M checked Rift `5.81 s`, `138 MB` RSS vs heap `6.50 s`, `784 MB`; 5M checked Rift `27.86 s`, `138 MB` vs heap `28.77 s`, `1.54 GB`, heap max GC `1066.487 ms`, heap fails at `1G`. | These are the current positive real-streaming retained-state rows. The 5M Wikimedia row is mainly RSS/fixed-memory/GC-tail scale-up evidence because L2 loop throughput is essentially tied. They are local named kernels over public compressed inputs, not exact prior-system artifacts. |
