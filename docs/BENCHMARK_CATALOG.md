...
| Broom retained dataflow | Broom/Naiad-style timestamped dataflow | Timestamped aggregate and join retain ordinary event/value objects in per-timestamp dictionaries until notify/close. The q17 workload adds deterministic generated `Part`/`LineItem` records, per-part retained aggregates, and a TPC-H-Q17-like below-average quantity/revenue filter at timestamp close. Q17 can also read DBGEN/TPC-H `part.tbl` and `lineitem.tbl` with `BROOM_Q17_INPUT_MODE=tpch-file`. | Prior-work-style headline row: natural heap/GC versus checked Rift, with checked scoped as the best-safe-region comparison row. At 20M active-16, aggregate checked Rift is `8.48 s`, `53 MB` RSS vs heap `10.78 s`, `236 MB`, with heap L2 GC `953.153 ms`; join checked Rift is `8.09 s`, `57 MB` vs heap `9.88 s`, `438 MB`, with heap L2 GC `486.649 ms`. Q17 active-16 20M is checked Rift `9.67 s`, `50 MB` RSS vs heap `14.45 s`, `232 MB`, with heap L2 GC `1370.380 ms`. Aggregate/join have fixed-memory failure evidence; generated q17 is throughput/GC/RSS evidence because heap caps down to `256M` complete. DBGEN SF0.1 q17 is now checked Rift `5.15 s`, `50.9 MB` RSS vs heap `5.57 s`, `257.5 MB`, and heap fails at `64M`; SF1/full TPC-H remains pending. Evidence is in `evidence/BROOM_RETAINED_DATAFLOW_MATRIX.md`. |
