...
| Real streaming-input replay | Does the result still hold when real records are consumed incrementally without preloading the full input? | LogHub HDFS `streaming-file` top templates, GH Archive `streaming-file` q1/q2, and Theodolite `streaming-file` q2 started; StackExchange, SNAP/Yak, and DSPBench conversions planned |
...
| Theodolite power | Real UCI household power-meter trace, preloaded and streaming-input | Local single-process Theodolite UC2/UC4-style downsampling/hierarchical aggregation over real energy data. The older preloaded/full-local q2 row ties elapsed and removes heap GC, but RSS is similar. The new `THEODOLITE_POWER_INPUT_MODE=streaming-file` row parses records inside each run. At 1M q2, checked scoped epoch is an L1 throughput/RSS/fixed-memory win: `10.07 s`, `29.6 MB` RSS versus heap `10.92 s`, `147 MB`; L2 checked scoped is `2436.713 ms`, GC `54.154 ms` versus heap `2479.703 ms`, GC `143.088 ms`. Parser/string allocation remains visible in all modes, so this is real-streaming evidence but not a huge-GC flagship. |
...
2. Larger or richer Theodolite-style IoT input paired with real industrial-energy data, or larger machine/security traces. RIoTBench/MHEALTH is now wired but parked as zero-GC ceiling evidence; Theodolite source is cloned, and the local UCI power streaming-file row is a useful RSS/fixed-memory win but not a huge-GC flagship.
